...
- `/code-review ACSMS-SCR-XXX [BE|FE]` — Run the project's mandatory code-review checklist (Format / Security / Third-party / Source code + No-Hardcode + No-Inline-Message rules) against the source of a screen. Optional 2nd arg scopes to one side (`BE` or `FE`); omitted = both. Read-only; produces inline OK/NOK/NA table with file:line evidence + suggested diffs (not applied).
