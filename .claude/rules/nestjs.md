...
**MANDATORY — pass `manager` to `logCreate` / `logUpdate` / `logDelete`
when calling from inside `dataSource.transaction(...)`.** Without it
the audit INSERT routes through the standalone `AuditLogService` repo
(separate DB connection) and survives any rollback — producing an
orphan `t_log` row for a business write that never committed. The
audit trail then disagrees with real DB state, which is the most
dangerous failure mode for a system with compliance requirements.

`logError` is the inverse: it MUST NOT receive `manager`. It runs in
the `catch` block AFTER the transaction has already rolled back; a
manager-bound INSERT would also be discarded. The standalone INSERT
survives so the failure trace persists for debugging.

Use the shared `buildAuditCtx(session, req, SCREEN_NAME, TABLE_NAME,
targetId)` helper from `@/common/utils/audit-context` — never declare
a local `function buildAuditCtx(...)` in the service file.
  ...
  try {
    const saved = await this.dataSource.transaction(async (manager) => {
      ...
      await this.auditLog.logCreate(
        buildAuditCtx(session, req, SCREEN_NAME, TABLE_NAME, created.tankaId),
        created,
        manager,   // ← MUST pass — joins the audit INSERT to this tx
      );
      return created;
    });
    return saved;
  } catch (err) {
    // No `manager` — tx already rolled back; the error log must
    // survive on the standalone connection.
  ...
  try {
    return await this.dataSource.transaction(async (manager) => {
      ...
      await this.auditLog.logUpdate(
        buildAuditCtx(session, req, SCREEN_NAME, TABLE_NAME, updated.tankaId),
        before,
        updated,
        manager,   // ← MUST pass
      );
