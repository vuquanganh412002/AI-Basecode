# Project Structure — AgriNews_ACSMS

> Standard folder layout for the `AgriNews_ACSMS` monorepo: NestJS backend + Vue 3 frontend + Terraform infrastructure.
...
│   │   └── user.store.ts
│   │
│   ├── api/                           # Hand-written axios wrappers
│   │   ├── axios-instance.ts          # Shared axios + interceptors
│   │   ├── <tag>/<tag>.ts             # One file per BE controller tag
│   │   └── __tests__/<tag>.spec.ts    # Wrapper unit tests
│   │
...
- `api/` — hand-written axios wrappers per BE controller tag (`<tag>/<tag>.ts`). All views/composables/stores import from here — never raw `axios`/`fetch`.
