...
### Layer 4: FK reference in body (cross-tenant injection guard)

Layers 1-3 protect the **target** the user operates on. Layer 4 protects
**parent rows referenced by FK ids in the request body** — e.g. when
a `POST /shiten` carries `kanri_shiten_id` in the body, the service
MUST verify that kanri_shiten belongs to the caller's JA.

Without this layer, a CHUOKAI user (session.ja_id=1) can POST
`{ kanri_shiten_id: 13 }` where row 13 belongs to JA=2 → the new
shiten is stored with jaId=1 but its parent reference points at JA=2's
row → cross-tenant data corruption + tenant-id enumeration (the user
can probe which ids exist in other tenants by watching for 201 vs 400).

**Helper:** `fetchFkInJa(repo, idField, id, expectedJaId, '<label>')`
in `@/common/utils/data-scope`. Splits the two miss cases:

- Row **absent** (or `id` is null/undefined) → `BadRequestException`
  with the canonical inline-FK-guard message
  `'<label>IDが存在しません。'`.
- Row **exists but belongs to a different tenant** →
  `ForbiddenException` → `DATA_SCOPE_VIOLATION` (HTTP 403) with the
...
(attacker can probe `kanri_shiten_id = 1, 2, …` and see 400 vs 403 to
discover which ids exist in other tenants). End-user UX and audit-log
clarity outweigh the enumeration risk here. Layer 2 (single-record
access via URL `:id`) still masks as 404 because the URL itself names
the id under probe; FK references in a body are different — caller
already supplied the id, so 403 doesn't reveal much they couldn't
brute-force anyway.
...
await fetchFkInJa(
  ...
  'kanriShitenId',
  dto.kanri_shiten_id,
  sessionJaId,           // expectedJaId
  '管理支店',
);

// hanbaiten.service.ts — NICHINO_STAFF 代行入力 supported, effective
// JA already resolved earlier in the method
await fetchFkInJa(
  this.tankaRepo,
  'tankaId',
...
// UPDATE flow — bind to the EXISTING row's jaId, not session.ja_id,
// so NICHINO_* editing an arbitrary tenant's row stays in that tenant.
...
- Every FK id field arriving from a CREATE/UPDATE request body MUST go
  through `fetchFkInJa`. The previous pattern of plain
  `findOne({ where: { idField, deletedAt: IsNull() } })` is
  INSUFFICIENT — it accepts FK ids from other tenants.
- `expectedJaId`:
  - **CREATE, restricted-only endpoint** (shiten): `session.ja_id`.
  - **CREATE, allows NICHINO_STAFF 代行入力** (account, hanbaiten):
    effective JA — `dto.ja_id ?? session.ja_id`, resolved before the
    guard runs.
  - **UPDATE** (any role): `Number(before.jaId)` so the FK stays bound
    to the row's tenant even when editing.
- Wrap UPDATE-side calls in `if (dto.X !== undefined)` so partial
  PATCH-style updates don't trip the guard when the FK isn't being
  changed.
- Use `ForbiddenException` / `DATA_SCOPE_VIOLATION` for the
  cross-tenant FK case (customer decision 2026-05-19 — see above).
  Accept the tenant-id enumeration trade-off; UX and audit clarity
  outweigh the leak. The helper handles this — callers just pass
  `expectedJaId` and the helper picks 400 vs 403.
