...
ステップ2：
Tiêu đề trang "販売店情報登録", section thông tin cơ bản (販売店コード, 販売店名, カナ, 都道府県, 適格請求書発行事業者番号), section địa chỉ・liên lạc (郵便番号, 住所, 電話, FAX, 所長名), section thông tin ủy thác (委託区分, 配達料単価, 支払サイクル, 振込手数料負担区分, 振込手数料), section tài khoản chuyển khoản (銀行コード, 銀行名, 支店コード, 支店名, 預金種別, 口座番号, 口座名義), khác (廃店フラグ, 備考), button 登録/キャンセル đều được hiển thị
...
## ACSMS-TC-017-034 — Check giá trị mã master `furikomi_tesuryo_futan_kubun`
...
ステップ1：
Kiểm tra danh sách lựa chọn của dropdown 振込手数料負担区分

ステップ2：
Dùng DevTools gửi trực tiếp `furikomi_tesuryo_futan_kubun: 99`
...
## ACSMS-TC-017-035 — Check khoảng giá trị số `furikomi_tesuryo` (số âm NG)
...
ステップ1：
Nhập `-100` vào trường 振込手数料, click button đăng ký

ステップ2：
Nhập `0` vào trường 振込手数料, click button đăng ký
