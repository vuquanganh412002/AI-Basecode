...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 2026-05-07 | 1.0 | Tran Duc Tuyen | Tạo mới | Nguyen Huy Dat |  |
...
ステップ5：
HTTP 200, mảng `data` chứa toàn bộ record JA đã đăng ký, `meta.total` khớp với số bản ghi trong DB
...
ステップ2：
Hiển thị toast `アクセス権がありません。` + redirect về `/dashboard`

ステップ3：
HTTP 403, `error_code: FORBIDDEN`, message `この画面へのアクセス権限がありません。`
...
ステップ5：
HTTP 404, `error_code: NOT_FOUND`, message `指定されたJAが見つかりません。` (DataScope ẩn sự tồn tại)
...
ステップ2：
HTTP 200, search condition được áp dụng và màn hình refresh
...
ステップ2：
HTTP 200, search condition được áp dụng
...
ステップ2：
HTTP 200, áp dụng theo điều kiện AND
...
ステップ1：
HTTP 200, trả về `data: []`, `meta.total: 0`
...
ステップ1：
HTTP 200, search thực hiện bình thường

ステップ2：
HTTP 400, `error_code: VALIDATION_ERROR`, mảng `errors[]` chứa item `field: ja_code`

ステップ3：
HTTP 200, search thực hiện bình thường

ステップ4：
HTTP 400, `error_code: VALIDATION_ERROR`, mảng `errors[]` chứa item `field: ja_name`
...
ステップ1：
HTTP 200, `data: []` (không có record khớp)
...
ステップ1：
HTTP 200, `data: []` (không có record khớp) — SQL payload được truyền vào ILIKE clause như literal string

ステップ2：
HTTP 200, `data: []` — tương tự, không xảy ra việc xóa table
...
ステップ1：
HTTP 200, hiển thị 1〜20 trong 25 record
...
ステップ3：
HTTP 200, response `{ "message": "正常に削除しました" }`, hiển thị toast `JAを削除しました。`
...
ステップ3：
HTTP 409, `error_code: CONFLICT`, hiển thị toast message `このJAは関連オブジェクトに紐づいているため削除できません。`
...
ステップ1：
HTTP 404, `error_code: NOT_FOUND`, message `指定されたJAが見つかりません。`
...
ステップ2：
HTTP 403, `error_code: FORBIDDEN`, message `この画面へのアクセス権限がありません。` (CHUOKAI không có quyền `ja.delete`)
...
ステップ1：
Xóa thành công, hiển thị toast `JAを削除しました。`
...
ステップ1：
HTTP 500, `error_code: INTERNAL_SERVER_ERROR`, hiển thị toast `システムエラーが発生しました。しばらくしてから再度お試しください。`
...
ステップ1：
Chuyển trang fail, hiển thị toast `指定されたJAが見つかりません。`, vẫn ở màn search hoặc redirect về `/dashboard`
...
ステップ1：
HTTP 401, `error_code: UNAUTHORIZED`, message `セッションが切れました。再度ログインしてください。`
...
ステップ2：
HTTP 400, `error_code: BAD_REQUEST` hoặc `VALIDATION_ERROR`, message `リクエストパラメータが不正です。` hoặc `入力値が不正です。詳細はerrorsフィールドを確認してください。`
...
ステップ2：
HTTP 400, `error_code: VALIDATION_ERROR`, message `入力値が不正です。詳細はerrorsフィールドを確認してください。`
...
ステップ2：
HTTP 429, `error_code: TOO_MANY_REQUESTS`, message `リクエスト回数が上限を超えました。しばらくしてから再度お試しください。`
