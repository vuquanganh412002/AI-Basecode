...
ステップ5：
HTTP 404, `error_code: NOT_FOUND` (không phải 403 — out-of-scope ẩn sự tồn tại)
...
ステップ1：
HTTP 400, `error_code: DUPLICATE_CODE`, hiển thị message `同一の単価コードが既に登録されています。`, đăng ký thất bại
...
ステップ2：
Hiển thị toast thành công `登録しました。` + redirect về màn danh sách `/tanka`
...
ステップ1：
HTTP 400, `error_code: VALIDATION_ERROR`, response body chứa array `errors: [{ field, message }]`
...
・Rollback transaction, duy trì tính nhất quán dữ liệu
...
ステップ1：
Hiển thị toast thành công `更新しました。` + redirect về danh sách `/tanka`
...
ステップ2：
HTTP 400, `error_code: BAD_REQUEST`, message `リクエストパラメータが不正です。`
