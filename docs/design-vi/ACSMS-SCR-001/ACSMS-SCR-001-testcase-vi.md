...
補足：
・Sau đăng nhập thành công, toast「ログインしました。」hiển thị, chuyển về màn hình chính (/dashboard)
...
Toast「ログインしました。」hiển thị, chuyển về `/dashboard`
...
ステップ3：
mfa_token mất khỏi query parameter, onMounted của màn hình MFA chuyển về `/login`
