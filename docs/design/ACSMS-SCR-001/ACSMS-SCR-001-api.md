...
### 4.2 レスポンス生成
- `message: "ログアウトしました"` を返却する。HTTP 200。
...
| # | パラメーターID | タイプ | 繰り返し | 必須 | 最小長 | 最大長 | 説明 |
|---|---|---|---|---|---|---|---|
| 1 | limit | Number | - | - | | | 取得件数上限。デフォルト: 20、最大: 20 |
...
### 4.3 データ取得条件の設定
- 以下の条件でお知らせを取得する：
  - publish_location = 1（ログイン画面固定）
  - お知らせ種別 ∈ (1:システム, 2:重要, 3:一般)（4:締め切り時間は除外）
  - 状態 = 2（公開）
  - 公開開始日 <= 現在日時
...
### 4.4 データ取得
- 並び順は更新日時の降順。更新日時が NULL の場合は作成日時で代替する
  （`COALESCE(updated_at, created_at)`）。

```sql
SELECT oshirase_id, oshirase_type, title, publish_start_date
FROM t_oshirase
WHERE publish_location = 1
  AND oshirase_type IN (1, 2, 3)
  AND status = 2
