...
- NICHINO_ADMIN はデータスコープ制限なし（全JA管理支店にアクセス可能）。
...
| リクエストパラメーター | kanri_shiten_id（パスパラメータ） |
| ヘッダ | Content-Type: application/json\n※ 認証情報はHTTP-only Cookieにより自動的に送信される |
| HTTPレスポンスコード | 200:正常に管理支店を更新しました, 400:入力内容にエラーがあります, 401:セッションが切れました。再度ログインしてください, 403:この画面へのアクセス権限がありません, 404:指定された管理支店が見つかりません, 500:システムエラーが発生しました |
...
- 存在しない場合：HTTP 404 Not Found を返却する。
