...
| 10  | sort_by        | String  | -        | -    |        |        | ソート対象カラム（hanbaiten_code, hanbaiten_name）。デフォルト: hanbaiten_code                    |
| 11  | sort_order     | String  | -        | -    |        |        | ソート順（asc, desc）。デフォルト: asc                                                            |
...
| 15  | →tesuryo_kubun               | Number  | -        |              | 〇       | 振込手数料負担区分（※m_code.code_category='TESURYO_KUBUN'を参照、1:JA, 2:販売店）。一覧では「振込手数料負担区分」列に表示する |
...
- クエリパラメータの検証：
  ...
  - sort_by：許可カラム（hanbaiten_code, hanbaiten_name）のみ（画面設計書 v1.2 §8.1）。デフォルト: hanbaiten_code
  - sort_order：asc または desc のみ。デフォルト: asc
- 不正なパラメータが存在する場合：
...
SELECT h.hanbaiten_id, h.ja_id, h.hanbaiten_code, h.hanbaiten_name,
       ...
       h.tesuryo_kubun, h.tesuryo_amount, h.haiten_flg,
...
- 列ラベルとデータの対応（画面設計書 v1.2 §6 検索結果テーブル）：
  ...
  - 「振込手数料負担区分」列（旧「手数料区分」）→ `tesuryo_kubun`（FE は `useCodesStore().label('TESURYO_KUBUN', value)` でラベル解決）
