...
| 7   | →torihikisaki_no           | String | -        |              | -         | 適格請求書発行事業者番号   |
| 8   | →todofuken_code            | String | -        |              | -         | 都道府県コード（m_todofuken.todofuken_code、2桁。プルダウンは ACSMS-API-COMMON-001 を参照） |
| 9   | →yubin_no                  | String | -        |              | -         | 郵便番号                   |
| 10  | →address                   | String | -        |              | -         | 住所                       |
| 11  | →tel                       | String | -        |              | -         | 電話番号                   |
| 12  | →fax                       | String | -        |              | -         | FAX番号                    |
| 13  | →shocho_name               | String | -        |              | -         | 所長名                     |
| 14  | →itaku_kubun               | Number | -        |              | 〇       | 委託区分（※m_code.code_category='ITAKU_KUBUN'を参照、1:振込, 2:日農委託, 9:その他） |
| 15  | →haitatsuryo_tanka_id      | Number | -        |              | 〇       | 配達手数料単価ID（FK:m_tanka） |
| 16  | →haitatsuryo_shiharai_cycle| Number | -        |              | 〇       | 配達手数料支払サイクル（月数） |
| 17  | →tesuryo_kubun             | Number | -        |              | 〇       | 振込手数料負担区分（※m_code.code_category='TESURYO_KUBUN'を参照、1:JA, 2:販売店） |
| 18  | →tesuryo_amount            | Number | -        | 0.00         | 〇       | 手数料金額                 |
| 19  | →bank_code                 | String | -        |              | -         | 金融機関コード             |
| 20  | →bank_name                 | String | -        |              | -         | 金融機関名                 |
| 21  | →bank_branch_code          | String | -        |              | -         | 口座支店コード             |
| 22  | →bank_branch_name          | String | -        |              | -         | 口座支店名                 |
| 23  | →yokin_shubetsu            | Number | -        |              | 〇       | 口座種別（※m_code.code_category='YOKIN_SHUBETSU'を参照、1:普通, 2:当座） |
| 24  | →koza_no                   | String | -        |              | -         | 口座番号                   |
| 25  | →koza_meigi                | String | -        |              | -         | 口座名義                   |
...
SELECT hanbaiten_id, ja_id, hanbaiten_code, hanbaiten_name,
       ...
       itaku_kubun, haitatsuryo_tanka_id,
...
| 14  | tesuryo_kubun            | Number | -        | -    |        |        | 振込手数料負担区分（※m_code.code_category='TESURYO_KUBUN'を参照、1:JA, 2:販売店） |
| 15  | tesuryo_amount           | Number | -        | -    |        |        | 手数料金額 ≧ 0                          |
...
- リクエストボディの検証：
  ...
  - tesuryo_kubun：※m_code.code_category='TESURYO_KUBUN'を参照（1, 2 のいずれか）
  - tesuryo_amount：数値型チェック、≧ 0
  - haiten_flg：Boolean型チェック（省略時は false）
  - biko：テキスト型
- **委託区分による条件付き必須チェック（画面設計書 v1.2 §3.1）：**
  - itaku_kubun = 1（振込）の場合、以下を全て必須とする（No.17～No.23 必須）：
    - bank_code：必須、最大4桁
    - bank_name：必須、最大100桁
    - bank_branch_code：必須、最大3桁
    - bank_branch_name：必須、最大100桁
    - yokin_shubetsu：必須、※m_code.code_category='YOKIN_SHUBETSU'を参照（1 または 2）
    - koza_no：必須、最大10桁
    - koza_meigi：必須、最大50桁
  - itaku_kubun = 2（日農委託）または 9（その他）の場合：No.17～No.23 は空白可（任意）
...
| リクエストパラメーター | hanbaiten_id（パスパラメータ）                                                                                                                                                                                                     |
| ヘッダ                 | Content-Type: application/json  ※ 認証情報はHTTP-only Cookieにより自動的に送信される                                                                                                                                             |
| HTTPレスポンスコード   | 200:更新しました, 400:入力内容にエラーがあります, 401:セッションが切れました。再度ログインしてください, 403:この画面へのアクセス権限がありません, 404:指定された販売店が見つかりません, 500:システムエラーが発生しました |
...
| 21  | koza_no                  | String | -        | -    | 1      | 10     | 口座番号（itaku_kubun=1の場合は必須）    |
| 22  | koza_meigi               | String | -        | -    | 1      | 50     | 口座名義（itaku_kubun=1の場合は必須）    |
| 23  | haiten_flg               | Boolean| -        | -    |        |        | 廃店フラグ（true:廃店, false:営業中）   |
| 24  | biko                     | String | -        | -    |        |        | 備考                                     |
