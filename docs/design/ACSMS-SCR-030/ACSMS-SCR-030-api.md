...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API名                  | Export Log CSV                                                                                                                                                                                                                                          |
| 概要                   | **画面に表示中のページ**（一覧と同一の検索条件・並び順・page・per_page）のみをCSV形式で出力する（全件出力ではない）。1ページは per_page（最大100件）に収まるため件数上限・409は不要。 |
| URI                    | /api/v1/log/export                                                                                                                                                                                                                                      |
| メソッド               | GET                                                                                                                                                                                                                                                     |
| リクエストボディー     | なし                                                                                                                                                                                                                                                    |
| リクエストパラメーター | クエリパラメータ                                                                                                                                                                                                                                        |
| ヘッダ                 | Content-Type: application/json※ 認証情報はHTTP-only Cookieにより自動的に送信される                                                                                                                                                                  |
| HTTPレスポンスコード   | 200:正常にCSVをダウンロードしました, 401:セッションが切れました。再度ログインしてください, 403:この画面へのアクセス権限がありません, 400:入力値が不正です, 500:システムエラーが発生しました |

## リクエストパラメータ

一覧 (API-030-001) と同一。表示中ページを再現するため page / per_page / sort_by / sort_order を含む。
...
| 3   | log_type       | Number | -        | -    |        |        | ログ種別（1:ユーザー操作, 2:システム, 3:エラー, 4:ファイルアップロード、未指定=すべて）|
| 4   | account_id     | Number | -        | -    |        |        | アカウントID（ユーザー絞り込み）                                                       |
| 5   | page           | Number | -        | -    |        |        | ページ番号（デフォルト: 1）。表示中ページと一致させる                                  |
| 6   | per_page       | Number | -        | -    |        |        | 1ページの件数（デフォルト: 20、最大: 100）                                             |
| 7   | sort_by        | String | -        | -    |        |        | 並び替えカラム（log_datetime / log_type / result_status、デフォルト: log_datetime）    |
| 8   | sort_order     | String | -        | -    |        |        | 並び順（asc / desc、デフォルト: desc）                                                 |
...
- 一覧 (API-030-001) と同一の検索条件・DataScope・並び順で、**現在ページのみ**を
  `LIMIT :per_page OFFSET (:page - 1) * :per_page` で取得する（全件出力しない）。
- 1ページは per_page（最大100）に収まるため、件数上限チェック・409は行わない。
