...
|---|---|---|---|---|---|---|---|
| 1 | tanka_type | Number | - | - | | | 単価種別（完全一致） |
| 2 | tanka_name | String | - | - | | 100 | 単価名（部分一致） |
| 3 | page | Number | - | - | | | ページ番号（デフォルト: 1） |
| 4 | per_page | Number | - | - | | | 1ページの件数（デフォルト: 20） |
| 5 | sort_by | String | - | - | | | ソート項目（m_tankaテーブルのカラム名を指定。例: tanka_code, tanka_name, kingaku_zeikomi, tekiyo_start_date） |
...
| 1 | data | Array | 〇 | | - | |
| 2 | →tanka_id | Number | - | | - | |
| 3 | →tanka_type | Number | - | | - | 1: 新聞購読料, 2: 配達手数料 |
| 4 | →tanka_type_label | String | - | | - | tanka_typeのラベル |
| 5 | →tanka_code | String | - | | - | |
| 6 | →tanka_name | String | - | | - | |
...
| 10 | meta | Object | - | | - | |
| 11 | →total | Number | - | | - | 総件数 |
| 12 | →page | Number | - | | - | 現在のページ番号 |
| 13 | →per_page | Number | - | | - | 1ページの件数 |
| 14 | →total_pages | Number | - | | - | 総ページ数 |
...
| リクエストパラメーター | |
| ヘッダ | Content-Type: application/json\n※ 認証情報はHTTP-only Cookieにより自動的に送信される |
| HTTPレスポンスコード | 200:正常に削除しました, 400:リクエストパラメータが不正です, 401:セッションが切れました。再度ログインしてください, 403:この画面へのアクセス権限がありません, 404:指定された単価が見つかりません, 409:関連データが存在するため削除できません, 500:システムエラーが発生しました |
