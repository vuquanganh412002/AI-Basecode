...
| 8 | sort_by | String | - | - | | | ソート項目（m_tankaテーブルのカラム名を指定。例: tanka_code, tanka_name, kingaku_zeikomi, tekiyo_start_date） |
...
| 12 | meta | Object | - | | - | |
| 13 | →total | Number | - | | - | 総件数 |
| 14 | →page | Number | - | | - | 現在のページ番号 |
| 15 | →per_page | Number | - | | - | 1ページの件数 |
| 16 | →total_pages | Number | - | | - | 総ページ数 |
