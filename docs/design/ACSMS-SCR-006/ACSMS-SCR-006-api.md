...
| --- | -------------- | ------ | -------- | ---- | ------ | ------ | ---------------------------------------------------------------------------------- |
| 1   | shiten_name    | String | -        | -    |        | 100    | 支店名（部分一致検索）                                                             |
| 2   | page           | Number | -        | -    |        |        | ページ番号（デフォルト: 1）                                                        |
| 3   | per_page       | Number | -        | -    |        |        | 1ページの件数（デフォルト: 20、最大: 100）                                         |
| 4   | sort_by        | String | -        | -    |        |        | ソート項目（shiten_code, shiten_name。デフォルト: shiten_code）                    |
| 5   | sort_order     | String | -        | -    |        |        | ソート方向（asc / desc。デフォルト: asc）                                          |
...
```
GET /api/v1/shiten?shiten_name=本店&page=1&per_page=20&sort_by=shiten_code&sort_order=asc
```
...
- クエリパラメータの検証：
  ...
  - sort_by：許可値チェック（shiten_code, shiten_name）
  - sort_order：許可値チェック（asc, desc）
- デフォルト値の適用：
  - page：未指定の場合、1
  - per_page：未指定の場合、20
  - sort_by：未指定の場合、shiten_code
