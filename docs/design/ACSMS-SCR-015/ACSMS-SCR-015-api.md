...
| #   | パラメーターID    | タイプ | 必須 | 最小長 | 最大長 | 説明                          |
| --- | ----------------- | ------ | ---- | ------ | ------ | ----------------------------- |
| 1   | kanri_shiten_id   | Number | -    | -      | -      | 管理支店IDで絞込み（推奨）   |
...
```
GET /api/v1/shiten/dropdown?kanri_shiten_id=10
```
...
| #   | パラメーターID    | タイプ | 必須 | 最小長 | 最大長 | 説明                                                |
| --- | ----------------- | ------ | ---- | ------ | ------ | --------------------------------------------------- |
| 1   | search            | String | -    | -      | 100    | 検索文字列（販売店コード or 販売店名の部分一致）   |
...
```
GET /api/v1/hanbaiten/dropdown?search=千代田
```
...
{
  "data": [
    { "hanbaiten_id": 200, "hanbaiten_code": "H001", "hanbaiten_name": "千代田販売店", "ja_id": 100 },
    { "hanbaiten_id": 201, "hanbaiten_code": "H002", "hanbaiten_name": "中央販売店", "ja_id": 100 }
  ]
}
```
...
```sql
SELECT h.hanbaiten_id, h.hanbaiten_code, h.hanbaiten_name, h.ja_id
FROM m_hanbaiten h
WHERE h.deleted_at IS NULL
  /* DataScope */
  AND h.ja_id = :user_ja_id
  /* 検索条件 */
  AND (:search IS NULL OR h.hanbaiten_code LIKE :search_like OR h.hanbaiten_name LIKE :search_like)
