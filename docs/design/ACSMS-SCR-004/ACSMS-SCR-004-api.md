...
| メソッド               | GET                                                                                                                                                                                                  |
| リクエストボディー     | なし                                                                                                                                                                                                 |
| リクエストパラメーター | ?ja_code={ja_code}&ja_name={ja_name}&todofuken_code={todofuken_code}&page={page}&per_page={per_page}&sort_by={sort_by}&sort_order={sort_order}                                                       |
...
| 9   | →address        | String | -        |              | -         | 住所                              |
| 10  | →fax            | String | -        |              | -         | FAX番号                           |
| 11  | →chuokai_flg    | Boolean | -       |              | -         | 中央会フラグ（true:中央会, false:JA） |
| 12  | →jastem_itakusha_code | String | - |              | -         | JASTEM_委託者コード ※空文字許容    |
| 13  | →jastem_itakusha_name | String | - |              | -         | JASTEM_委託者名 ※空文字許容        |
| 14  | →jastem_ja_code | String | -        |              | -         | JASTEM_農協番号 ※空文字許容        |
| 15  | →jastem_ja_name | String | -        |              | -         | JASTEM_農協名 ※空文字許容          |
...
- 検索条件：
  ...
  - todofuken_code：完全一致（`mj.todofuken_code = :todofuken_code`）
...
WHERE mj.deleted_at IS NULL
  ...
  AND (:todofuken_code IS NULL OR mj.todofuken_code = :todofuken_code)
...
| リクエストパラメーター |                                                                                                                                                                                                                                                                            |
| ヘッダ                 | Content-Type: application/json\n※ 認証情報はHTTP-only Cookieにより自動的に送信される                                                                                                                                                                                       |
| HTTPレスポンスコード   | 200:削除しました, 400:リクエストパラメータが不正です, 401:セッションが切れました。再度ログインしてください, 403:この画面へのアクセス権限がありません, 404:指定されたJAが見つかりません, 409:関連データが存在するため削除できません, 500:システムエラーが発生しました |
