...
| 7 | 2026/08/18 | 1.16 | Tran Duc Tuyen | 顧客要件2026-08：t_dokusya_rireki に hanbaiten_tohaigo_flg（販売店統廃合フラグ、DEFAULT false, TRUE=販売店統廃合に伴う販売店変更）を追加し、以降のNoを再採番。購読者販売店一括置換画面での変更=TRUE、購読者情報登録画面での変更=FALSE。TRUEの履歴は増減連絡票（販売店）の出力対象外とする。増減通知（日本農業新聞）は仕様変更なし | Nguyen Huy Dat | Nguyen Huy Dat |
...
| 75 | hanbaiten_tohaigo_flg |  | BOOLEAN |  |  |  | 販売店統廃合フラグ（DEFAULT false, TRUE=販売店統廃合に伴う販売店変更）<br>購読者販売店一括置換画面での変更=TRUE、購読者情報登録画面での変更=FALSE。<br>TRUEの履歴は増減連絡票（販売店）の出力対象外とする。 |
| 76 | created_at |  | TIMESTAMPTZ |  |  |  | 作成日時（履歴登録日時） |
| 77 | created_by |  | VARCHAR | 50 |  |  | 作成者（履歴登録者） |
