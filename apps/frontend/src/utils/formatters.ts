...
/**
 * 管理支店コード canonical format — three groups of HALF-WIDTH DIGITS
 * separated by hyphens (length 3-4-3 = e.g. "013-3300-001"). Customer
 * spec rejects any non-digit character (alphabet was previously
 * accepted by mistake — fixed 2026-05-19).
 */
export const KANRI_SHITEN_CODE_REGEX = /^\d{3}-\d{4}-\d{3}$/;

/**
 * Auto-insert hyphens when the user types the bare 10-digit form.
 * If the input is already in `XXX-XXXX-XXX` shape, returns it unchanged;
 * if it doesn't fit either shape, returns the original so the caller's
 * `KANRI_SHITEN_CODE_REGEX` check can flag it as invalid.
 *
 *   formatKanriShitenCode('0133300001') -> "013-3300-001"
 *   formatKanriShitenCode('013-3300-001') -> "013-3300-001"
 *   formatKanriShitenCode('abc1234567') -> "abc1234567"   (caller validates)
 *   formatKanriShitenCode('short') -> "short"             (caller validates)
 */
