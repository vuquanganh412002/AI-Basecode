...
const STAFF_PERMS = [
  'hanbaiten.create', 'hanbaiten.view', 'hanbaiten.update', 'hanbaiten.import',
...
describe('AppSidebar — permission-driven visibility', () => {
  ...
  describe('日農（担当者） / NICHINO_STAFF (role_id=2)', () => {
    ...
    it('should show hanbaiten + daiko_input + common menus when role is NICHINO_STAFF', () => {
      expect(labels).toContain('メニュー画面');
      ...
      expect(labels).toContain('ファイルダウンロード');
      expect(labels).toContain('ログ参照');
      expect(labels).toContain('販売店代行入力');
    });
