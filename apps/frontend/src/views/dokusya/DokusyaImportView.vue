...
/** Whether a given column is required (always checked) in the current mode. */
function isRequired(col: PhysicalColumn): boolean {
  return importModeFe.value === 'new' && REQUIRED_SET.has(col);
}
...
const allChecked = computed<boolean>({
  ...
  set: (value: boolean) => {
    for (const col of PHYSICAL_COLUMNS) {
      // NEW-mode required columns stay checked even on uncheck-all.
      selected[col] = isRequired(col) ? true : value;
    }
  },
});
...
async function runImport(): Promise<void> {
  ...
  try {
    ...
    // Render row-level errors for IMPORT_VALIDATION_ERROR. The global
    // axios interceptor toasts FORBIDDEN / 500 centrally — the view must
    // NOT re-toast those (it only surfaces the row list here).
    const body = (err as { response?: { data?: unknown } })?.response?.data as
      | { error_code?: string; errors?: RowError[] }
      | undefined;
    if (body?.error_code === 'IMPORT_VALIDATION_ERROR' && Array.isArray(body.errors)) {
...
onMounted(() => {
  ...
  for (const col of REQUIRED_COLUMNS_NEW) {
    ...
    Object.defineProperty(el, 'checked', {
      configurable: true,
      enumerable: true,
      get: () => isRequired(col) || selected[col],
      set: () => {
          ...
          >
            ...
            >
              <input
                ...
                type="checkbox"
                class="w-3.5 h-3.5 rounded border-border-strong accent-primary focus:ring-primary/20"
              />
              すべて選択／解除
            </label>
          </div>
