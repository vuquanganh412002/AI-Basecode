...
// ═══════════════════════════════════════════════════════════════════════
// 14. 購読種別=電子版 → 支払方法 restricted to 口座引落 (create mode)
// ═══════════════════════════════════════════════════════════════════════
describe('DokusyaFormView — 電子版 restricts 支払方法 (create mode)', () => {
  it('should reset 支払方法 to null when 購読種別 changes to 電子版 with a non-口座引落 value', async () => {
    const { wrapper } = await renderView(); // create mode
    const vm = wrapper.vm as any;
    vm.formState.shiharai_hoho = 2; // 現金集金 (not allowed for 電子版)
