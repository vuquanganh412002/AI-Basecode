...
// ═══════════════════════════════════════════════════════════════════════
// 12. 履歴表示 (機能定義 5.x + 13.x)
// ═══════════════════════════════════════════════════════════════════════
describe('DokusyaFormView — history button (機能定義 5.x / 13.x)', () => {
  it('should call getDokusyaHistory when 履歴表示 is clicked in edit mode', async () => {
    const { wrapper } = await renderView({ dokusyaId: 100 });
    const { getDokusyaHistory } = await import('@/api/dokusya/dokusya');
    vi.mocked(getDokusyaHistory).mockClear();
    ...
    expect(getDokusyaHistory).toHaveBeenCalledTimes(1);
    expect(vi.mocked(getDokusyaHistory).mock.calls[0]?.[0]).toBe(100);
  });

  it('should render history rows ordered by rireki_no DESC when 履歴表示 is clicked (機能定義 5.2)', async () => {
    const { wrapper } = await renderView({ dokusyaId: 100 });
    ...
    // The 2-row history fixture sets rireki_no=2 first then 1; assert
    // both surface in the rendered text in that order.
    const text = wrapper.text();
    const idx2 = text.indexOf('住所変更');
    const idx1 = text.indexOf('新規');
    expect(idx2).toBeGreaterThan(-1);
    expect(idx1).toBeGreaterThan(-1);
    expect(idx2).toBeLessThan(idx1);
  });

  it('should NOT call getDokusyaHistory when mounted in create mode (button not rendered)', async () => {
    await renderView();
    const { getDokusyaHistory } = await import('@/api/dokusya/dokusya');
    expect(getDokusyaHistory).not.toHaveBeenCalled();
  });
});
