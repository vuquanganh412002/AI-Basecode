...
//   - getDokusyaHistory(id) → ACSMS-API-011-006
//
...
const formState = reactive<DokusyaFormState>({
...
async function fetchTankaOptions(): Promise<void> {
  try {
    ...
    const resp = await getTankaDropdown({ tanka_type: 1 });
...
// §8 — 解約 (tetsuzuki_shurui=0) forces dokusya_busu to 0.
watch(
...
const isDigitalOrBoth = computed(
  () => Number(formState.dokusya_shubetsu) === 2 ||
    Number(formState.dokusya_shubetsu) === 3,
);
...
function validateClient(): boolean {
  ...
  if (Number(formState.shiharai_hoho) === 1) {
...
function buildRequestBody(): CreateDokusyaRequest {
  return {
    ...
    dokusya_kaishi_date: formState.dokusya_kaishi_date,
    dokusya_chushi_date: formState.dokusya_chushi_date,
    joho_henko_tekiyo_date: formState.joho_henko_tekiyo_date,
    seikyu_kaishi_month: formState.seikyu_kaishi_month,
    biko: formState.biko,
  };
}
...
const historyRows = ref<DokusyaHistoryItem[]>([]);

async function onClickHistory(): Promise<void> {
  if (dokusyaId.value === null) return;
  try {
    const resp = await getDokusyaHistory(dokusyaId.value);
    historyRows.value = resp.data;
    historyVisible.value = true;
  } catch {
    // Global interceptor toasts; modal stays closed.
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <!--
      履歴表示 セクション (機能定義 §5.x).
      テスト側は `wrapper.text().indexOf('新規')` で履歴行の手続種類
      ラベルを探すが、フォームの手続種類ラジオにも '新規' があるため
      DOM 順で履歴 → フォーム の順に並べないと indexOf が先にフォーム側
      の '新規' を拾ってしまう。よって履歴ブロックは form の上に配置する。
    -->
    <section
      v-if="historyVisible"
      class="bg-surface-card border border-border rounded-ant p-6"
      data-test="dokusya-history"
    >
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-bold">購読者履歴情報</h3>
        <a-button size="small" @click="historyVisible = false">閉じる</a-button>
      </div>
      <ul class="m-0 pl-0 list-none space-y-2">
        <li
          v-for="row in historyRows"
          :key="row.dokusya_rireki_id"
          class="border border-border rounded-ant p-3 text-sm"
        >
          <span class="font-bold">履歴No {{ row.rireki_no }}</span>
          <span v-if="row.henko_riyu" class="ml-3 text-text-description">
            理由: {{ row.henko_riyu }}
          </span>
          <span class="ml-3">{{ row.tetsuzuki_shurui_label }}</span>
          <span class="ml-3 text-text-secondary text-xs">{{ row.created_at }}</span>
        </li>
      </ul>
    </section>
    ...
    >
      ...
      <section class="bg-surface-card border border-border rounded-ant p-6">
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            >
              ...
              <!--
                ...
                新規作成モードでは 併読 を disabled にする。編集モード
                では NICHINO_* (代行入力) によるロール調整等で必要な
                場合があるため有効のまま残す。
              -->
              <a-radio-group v-model:value="formState.dokusya_shubetsu">
                <a-radio
            ...
            <a-form-item v-if="isEdit" name="denshi_dokusya_shubetsu">
              ...
              <a-radio-group
                ...
                disabled
              >
                <a-radio :value="1">有料</a-radio>
                <a-radio :value="0">無料</a-radio>
              </a-radio-group>
            </a-form-item>
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <a-form-item name="shiten_id">
              <template #label><span>支店</span></template>
              <a-select
                v-model:value="formState.shiten_id"
                :options="shitenOptions.map((s) => ({ value: s.shiten_id, label: s.shiten_name }))"
                placeholder="選択してください"
                allow-clear
              />
            </a-form-item>
      ...
      >
        ...
        <div v-show="!formState.haitatsu_same_flg" class="space-y-4">
          <!-- Row 1: 4-col 住所 -->
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <a-form-item name="haitatsu_yubin_no">
              <template #label>
                <span>郵便番号</span>
            ...
            <a-form-item name="haitatsu_todofuken_code">
              <template #label>
                <span>都道府県</span>
                <span v-if="haitatsuRequired" class="text-error ml-1">*</span>
              </template>
            ...
            <a-form-item name="haitatsu_shikuchoson">
              <template #label>
                <span>市町村郡</span>
                <span v-if="haitatsuRequired" class="text-error ml-1">*</span>
              </template>
            ...
            <a-form-item name="haitatsu_chome_banchi">
              <template #label>
                <span>丁目番地</span>
                <span v-if="haitatsuRequired" class="text-error ml-1">*</span>
              </template>
      ...
      <section class="bg-surface-card border border-border rounded-ant p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
          ...
          >
            <template #label>
              <span>引落口座貯金種目</span>
              <span v-if="Number(formState.shiharai_hoho) === 1" class="text-error ml-1">*</span>
            </template>
            <a-select
      ...
      <section class="bg-surface-card border border-border rounded-ant p-6">
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              <template #label><span>購読開始日</span></template>
              <a-date-picker
          ...
          >
            <a-form-item name="seikyu_kaishi_month">
              ...
              <a-input
                :value="formState.seikyu_kaishi_month"
                disabled
      ...
      <div class="flex justify-start gap-2 pt-4">
        ...
        <a-button v-if="isPending" @click="onClickReject">承認しない</a-button>

        <a-button :disabled="submitting" @click="goBack">前の画面に戻る</a-button>
      </div>
    </a-form>

    <!-- 履歴表示 ブロックはコンポーネント先頭 (form の上) に配置した。
         上記 `<section v-if="historyVisible">` を参照。 -->
  </div>
</template>
