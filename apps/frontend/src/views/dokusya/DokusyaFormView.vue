...
const shiharaiHohoOptions = computed(() => {
  ...
  // Create mode: 電子版 (dokusya_shubetsu=2) only allows 口座引落 (1).
  // クレジットカード / others come from the e-subscription system and are
  // not picked manually here. Edit keeps the full list because an existing
  // 電子版 record may legitimately carry クレカ from that system.
  if (!isEdit.value && Number(formState.dokusya_shubetsu) === DokusyaShubetsu.DIGITAL) {
    return all.filter((o) => Number(o.value) === ShiharaiHoho.KOZA_HIKIOTOSHI);
  }
  return all;
});

// When 購読種別 switches to 電子版 in create mode, drop a 支払方法 value
// that is no longer selectable (only 口座引落 remains) so the field never
// keeps a stale/invalid選択.
watch(
