...
describe('LoginView', () => {
  beforeEach(() => {
    ...
    vi.mocked(authApi.fetchLoginOshirase).mockResolvedValue([]);
  });
  ...
  describe('お知らせ list (§14-15)', () => {
    it('should call fetchLoginOshirase with limit=20 when mounted', async () => {
      await renderView();
      await flushPromises();
      expect(authApi.fetchLoginOshirase).toHaveBeenCalledWith(20);
    });
    ...
    it('should NOT block login form when fetchLoginOshirase rejects (§14.4)', async () => {
      vi.mocked(authApi.fetchLoginOshirase).mockRejectedValue({
