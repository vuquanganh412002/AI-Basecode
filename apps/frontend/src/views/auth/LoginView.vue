...
import { fetchPublicOshirase, type PublicOshiraseItem } from '@/api/auth/auth';
...
onMounted(async () => {
  try {
    const items = await fetchPublicOshirase(1, 10);
    notices.value = items.map((n: PublicOshiraseItem) => ({
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <div class="flex items-baseline gap-2">
              ...
              <span class="text-[10px] text-text-secondary">（平日 9:30〜17:30）</span>
            </div>
          </div>
        </div>
      </BaseCard>
