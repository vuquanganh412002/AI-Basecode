...
describe('KanriShitenListView — clear search (§3)', () => {
  ...
  it('should reset to page 1 when クリア is clicked', async () => {
    ...
    if (vm.state) vm.state.filters.kanri_shiten_code = 'x';
    await flushPromises();
...
// ───────────────────────────────────────────────────────────────────────
describe('KanriShitenListView — empty 検索 is a no-op', () => {
  it('should NOT call listKanriShiten when 検索 is submitted with all filters empty', async () => {
    const { wrapper } = await renderView();
    const { listKanriShiten } = await import('@/api/kanri-shiten/kanri-shiten');
    vi.mocked(listKanriShiten).mockClear(); // drop the onMounted fetch
    await wrapper.find('form').trigger('submit');
    await flushPromises();
    expect(listKanriShiten).not.toHaveBeenCalled();
  });

  it('should NOT call listKanriShiten when 検索クリア is clicked on a pristine screen', async () => {
    const { wrapper } = await renderView();
    const { listKanriShiten } = await import('@/api/kanri-shiten/kanri-shiten');
    vi.mocked(listKanriShiten).mockClear(); // drop the onMounted fetch
