...
describe('LogListView — clear search (機能定義 3.x)', () => {
  it('should refetch the default log list when 検索クリア is clicked', async () => {
    ...
    (wrapper.vm as unknown as { state: { filters: { log_type: number | null } } })
      .state.filters.log_type = 1;
...
describe('LogListView — CSV export (機能定義 4.x)', () => {
  ...
  it('should export the CURRENT page — send page + per_page (+ sort) with the filters', async () => {
    // CSV export mirrors the on-screen page, so it must carry the same
    // pagination/sort as the list (not export all data).
    const { wrapper } = await renderView();
    const { exportLogCsv } = await import('@/api/log/log');
    vi.mocked(exportLogCsv).mockClear();
...
// ───────────────────────────────────────────────────────────────────────
describe('LogListView — empty 検索 is a no-op', () => {
  it('should NOT call listLogs when 検索 is submitted with all filters empty', async () => {
    const { wrapper } = await renderView();
    const { listLogs } = await import('@/api/log/log');
    vi.mocked(listLogs).mockClear(); // drop the onMounted fetch
    await wrapper.find('form').trigger('submit');
    await flushPromises();
    expect(listLogs).not.toHaveBeenCalled();
  });

  it('should NOT call listLogs when 検索クリア is clicked on a pristine screen', async () => {
    const { wrapper } = await renderView();
    const { listLogs } = await import('@/api/log/log');
    vi.mocked(listLogs).mockClear(); // drop the onMounted fetch
