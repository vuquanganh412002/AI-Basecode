...
describe('JaListView — clear search (機能定義 3.x)', () => {
  ...
  it('should reset to page 1 when 検索クリア is clicked', async () => {
    ...
    (wrapper.vm as unknown as { state: { filters: { ja_code: string } } })
      .state.filters.ja_code = 'x';
...
// ───────────────────────────────────────────────────────────────────────
describe('JaListView — empty 検索 is a no-op', () => {
  it('should NOT call listJa when 検索 is submitted with all filters empty', async () => {
    const { wrapper } = await renderView();
    const { listJa } = await import('@/api/ja/ja');
    vi.mocked(listJa).mockClear(); // drop the onMounted fetch
    await wrapper.find('form').trigger('submit');
    await flushPromises();
    expect(listJa).not.toHaveBeenCalled();
  });

  it('should NOT call listJa when 検索クリア is clicked on a pristine screen', async () => {
    const { wrapper } = await renderView();
    const { listJa } = await import('@/api/ja/ja');
    vi.mocked(listJa).mockClear(); // drop the onMounted fetch
  ...
  it('fetches once when 検索 is pressed after emptying the input by hand, then is a no-op', async () => {
    const { wrapper } = await renderView();
    const { listJa } = await import('@/api/ja/ja');
    const input = wrapper.find('input[type="text"]'); // ja_code

    // Search "abc" → list is now filtered.
    await input.setValue('abc');
    await wrapper.find('form').trigger('submit');
    await flushPromises();

    // User empties the input by hand and presses 検索 → must fetch once to
    // restore the full list (form blank but the displayed list is still abc).
    vi.mocked(listJa).mockClear();
    ...
    // Pressing 検索 again with the still-empty form is a no-op.
    vi.mocked(listJa).mockClear();
    await wrapper.find('form').trigger('submit');
    await flushPromises();
    expect(listJa).not.toHaveBeenCalled();
