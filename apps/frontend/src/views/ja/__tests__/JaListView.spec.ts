...
import JaListView from '../JaListView.vue';
import {
  buildJaListItem,
  buildJaListResponse,
  buildAuthUser,
...
beforeEach(async () => {
  ...
  vi.mocked(removeJa).mockResolvedValue({ message: '正常に削除しました。' });
});
...
describe('JaListView — delete (機能定義 5.x)', () => {
  ...
  it('should display ACSMS-MSG-004-006 success toast when delete succeeds', async () => {
    ...
    // useNotify().deleted('JA') → 'JAを削除しました' OR
    // direct message.success('JAを削除しました。') — both satisfy the spec.
