...
const { state, loading, total, onChange, applyFilters, resetFilters } =
  useTableQuery<JaFilters>({
    defaultFilters: { ja_code: '', ja_name: '' },
...
function onSearch(): void {
  ...
  // visibly reflects the trimmed value too.
...
function goEdit(row: JaListItem): void {
  void router.push({ name: 'JaEdit', params: { ja_id: row.ja_id } });
}
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'chuokai_flg'">
          {{ (record as JaListItem).chuokai_flg ? '中央会' : '単協' }}
        </template>
