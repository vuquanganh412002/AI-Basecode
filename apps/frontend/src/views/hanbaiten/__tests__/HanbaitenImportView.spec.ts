...
describe('HanbaitenImportView (ACSMS-SCR-019) — file selection + preview', () => {
  ...
  it('should uncheck every NON-required column when the すべて選択 toggle is unchecked', async () => {
    ...
    await flushPromises();
    // 販売店コード stays checked (required); every other column flips off.
...
describe('HanbaitenImportView (ACSMS-SCR-019) — success path', () => {
  it('should show success toast with ACSMS-MSG-007-004 literal when import resolves with 200', async () => {
    ...
    await wrapper.find('[data-test="import-submit-btn"]').trigger('click');
    await flushPromises();
    expect(vi.mocked(message.success)).toHaveBeenCalledWith('正常に取り込みました。');
  });
...
describe('HanbaitenImportView (ACSMS-SCR-019) — error paths', () => {
  it('should render row-level errors when import rejects with IMPORT_VALIDATION_ERROR', async () => {
    ...
    await wrapper.find('[data-test="import-submit-btn"]').trigger('click');
    await flushPromises();
    // 機能 7.4 — 行{N}: {項目名} — {エラー理由}. Either an inline error
    // panel or a toast surfaces row 2 / row 3 + field names.
    const text = wrapper.text();
    const sawRowError =
      /行\s*2|行\s*3|hanbaiten_code|販売店コード|itaku_kubun/.test(text) ||
      vi.mocked(message.error).mock.calls
        .flatMap((c) => c)
        .some((m) => typeof m === 'string' && /行|hanbaiten_code|itaku/.test(m));
