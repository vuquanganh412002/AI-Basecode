  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE TABLE t_file_download (
        ...
        target_month VARCHAR(6) NOT NULL DEFAULT '',                        -- 対象年月（YYYYMM）※空文字許容
    ...
    await queryRunner.query(`COMMENT ON COLUMN t_file_download.ja_id IS 'JA ID（FK:m_ja）'`);
    ...
    await queryRunner.query(`COMMENT ON COLUMN t_file_download.target_month IS '対象年月（YYYYMM）※空文字許容'`);
