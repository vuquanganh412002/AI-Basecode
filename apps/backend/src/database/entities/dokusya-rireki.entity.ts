 ...
 *
 * Mirrors `docs/database/database-design.md §t_dokusya_rireki`. Each row
 * is an immutable snapshot of the parent `t_dokusya` at the moment of
 * a CREATE / UPDATE / 承認 / 否認 operation; the master row's
 * `rireki_no` always points at the row whose `saishin_data_flg = true`.
 *
 * The append-on-update pattern (api.md §4.4 ステップ1+2):
 *   1. UPDATE every existing rireki row's `saishin_data_flg = FALSE`
 *      so only the new row carries `saishin_data_flg = TRUE`.
 *   2. INSERT a new rireki row with `rireki_no = MAX(rireki_no) + 1`
 *      and `saishin_data_flg = TRUE`.
 *   3. UPDATE the master `t_dokusya.rireki_no` to that new number.
 *
