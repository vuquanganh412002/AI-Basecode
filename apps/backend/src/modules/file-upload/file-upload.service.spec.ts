...
describe('FileUploadService — SCR-023 (upload + delete + extended list)', () => {
  ...
  describe('upload', () => {
    ...
    it('should call StorageService.upload with ja-{ja_id}/files/{uuid}-{filename} path for each file', async () => {
      // COVERS: §4.4 — path format
      await service.upload(
        [12345],
    ...
    it('should set scheduled_delete_date to upload_datetime + 180 days', async () => {
      // COVERS: §4.5 — scheduled_delete_date = NOW() + INTERVAL '180 days'
      ...
      const savedCall = txManager.save.mock.calls.find(
        ([_e, v]: any[]) => v?.scheduledDeleteDate instanceof Date,
      );
      expect(savedCall).toBeDefined();
      const scheduled = savedCall![1].scheduledDeleteDate.getTime();
      const lowerBound = before + 180 * 24 * 60 * 60 * 1000 - 1000;
      const upperBound = after + 180 * 24 * 60 * 60 * 1000 + 1000;
      expect(scheduled).toBeGreaterThanOrEqual(lowerBound);
      expect(scheduled).toBeLessThanOrEqual(upperBound);
    });
