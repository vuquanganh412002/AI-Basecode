...
describe('OshiraseController — SCR-001 HTTP (public findLogin)', () => {
  ...
  describe('GET /api/v1/oshirase/login', () => {
    ...
    it('should default limit to 20 when omitted from query', async () => {
      service.findLogin.mockResolvedValue([]);
    ...
    it('should return 400 VALIDATION_ERROR when limit exceeds 20', async () => {
      await http()
...
describe('OshiraseController — SCR-010 HTTP (menu getMenuList)', () => {
  let app: INestApplication;
  let service: any;
  ...
  beforeEach(async () => {
    service = {
      getMenuList: jest
        .fn()
        .mockResolvedValue({ data: { oshirase_list: [], deadline_notice: null } }),
    };
    const allowAllGuard: CanActivate = { canActivate: () => true };
  ...
  describe('GET /api/v1/oshirase/menu', () => {
    it('should return 200 with { data: { oshirase_list, deadline_notice } }', async () => {
      service.getMenuList.mockResolvedValue({
        data: {
          oshirase_list: [{ oshirase_id: 1, title: 'お知らせ', oshirase_type: 1 }],
          deadline_notice: { oshirase_id: 9, title: '締め切り', oshirase_type: 4 },
        },
      });

      const res = await http().get('/api/v1/oshirase/menu').expect(200);

      expect(res.body.data.oshirase_list).toHaveLength(1);
      expect(res.body.data.deadline_notice).toMatchObject({ oshirase_id: 9 });
    });

    it('should forward a parsed numeric limit to the service', async () => {
      await http().get('/api/v1/oshirase/menu?limit=20').expect(200);
      // arg[0] is the session (undefined here — guard is an allow-all stub
      // that doesn't populate req.user); only the limit arg matters.
      expect(service.getMenuList.mock.calls[0][1]).toBe(20);
    });

    it('should pass undefined limit when the query omits it', async () => {
      await http().get('/api/v1/oshirase/menu').expect(200);
      expect(service.getMenuList.mock.calls[0][1]).toBeUndefined();
    });
  });
});
