...
describe('LogService', () => {
  ...
  describe('getLogList', () => {
    ...
    it('should apply pagination via take + skip when page=2 per_page=10', async () => {
      qbMock.getRawMany.mockResolvedValue([]);
      countQbMock.getCount.mockResolvedValue(0);
  ...
  describe('exportLogCsv', () => {
    ...
    it('should throw EXPORT_LIMIT_EXCEEDED when count exceeds 5000', async () => {
    ...
    it('should record record_count in after_value JSON when export succeeds (covers §4.6)', async () => {
      qbMock.getCount.mockResolvedValue(250);
      countQbMock.getRawMany.mockResolvedValue([]);
    ...
    it('should emit error log (log_type=3) OUTSIDE the transaction when export fails', async () => {
      // exportLogCsv createQB order: 1st=count (qbMock), 2nd=data (countQbMock).
      qbMock.getCount.mockResolvedValue(1);
      countQbMock.getRawMany.mockRejectedValueOnce(new Error('db-down'));
