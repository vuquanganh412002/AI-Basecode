...
describe('JaController (HTTP)', () => {
  ...
  describe('GET /api/v1/ja/:ja_id', () => {
    ...
    it.todo('should return 429 TOO_MANY_REQUESTS when throttler trips (covered in integration)');
  });
  ...
  describe('POST /api/v1/ja', () => {
    ...
    it('should return 201 with created JA when NICHINO_ADMIN posts valid body', async () => {
      ...
      expect(res.body.message).toBe('JAを登録しました。');
    });
    ...
    it('should return 400 VALIDATION_ERROR when bank_code is not 4 half-width digits', async () => {
      // COVERS: 4.1 bank_code 半角数字4桁 format check
      const res = await http()
        .post('/api/v1/ja')
        .send({ ...validBody, bank_code: 'abcd' })
        .expect(400);

      expect(res.body.error_code).toBe('VALIDATION_ERROR');
      expect(res.body.errors).toEqual(
        expect.arrayContaining([expect.objectContaining({ field: 'bank_code' })]),
      );
    });
  ...
  describe('PUT /api/v1/ja/:ja_id', () => {
    ...
    it('should return 200 with updated JA when NICHINO_ADMIN updates all fields', async () => {
      ...
      expect(res.body.data.ja_id).toBe(1);
      expect(res.body.message).toBe('JAを更新しました。');
    });
  ...
  describe('DELETE /api/v1/ja/:ja_id', () => {
    it('should return 200 with success message when NICHINO_ADMIN deletes a JA with no related data', async () => {
      // COVERS: happy path — 200, message='正常に削除しました。'
      service.remove.mockResolvedValue({ message: '正常に削除しました。' });
