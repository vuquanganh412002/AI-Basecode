...
import { SHUBETSU_MSG } from '@/modules/dokusya/dokusya-shubetsu.rules';
...
describe('DokusyaService — SCR-011 (create + update + approve/reject + history + detail)', () => {
  ...
  beforeEach(() => {
    ...
    dataSource = {
      ...
      query: jest.fn(async () => [{ count: '0' }]),
      // [reserved-same-date] assertReservedSameDateAvailable / getEffectiveAt
      // call loadActiveRowAtDate(this.dataSource.manager, ...) pre-transaction.
      // Default: no colliding row (most tests don't care about this rule).
      manager: {
        createQueryBuilder: jest.fn(() => ({
          ...makeQbMock(),
    ...
    // haitenFlg/activeFlg default to the "open store / active price" state
    // (DB column defaults: haiten_flg=false, active_flg=true) so existing
    // create/update tests keep passing; the dedicated 廃店/失効 tests below
    // override them per-case.
    hanbaitenRepo = {
      findOne: jest.fn(async () => ({ jaId: fkScopeJaId, haitenFlg: false })),
    };
    tankaRepo = {
      findOne: jest.fn(async () => ({ jaId: fkScopeJaId, activeFlg: true })),
    };
  ...
  describe('getEffectiveAt — joho vs 解約予定日 の早期検証（顧客要件2026-08）', () => {
    ...
    // 紙版の予約変更ポップアップ確定時点で「同一適用日に既存の変更がある」ことを
    // update() 送信より前に検知する（顧客要件2026-08）。
    it('should throw VALIDATION_ERROR(field=joho_henko_tekiyo_date) when an active row already exists at that exact date', async () => {
      dokusyaRepo.findOne.mockResolvedValue(
        buildDokusya({ dokusyaId: 7, jaId: 1, dokusyaShubetsu: 1, dokusyaKaishiDate: '2026-04-01' }),
      );
      rirekiRepo.findOne.mockResolvedValue(null);
      dataSource.manager = {
        createQueryBuilder: jest.fn(() => ({
          ...makeQbMock(),
          getOne: jest.fn().mockResolvedValue({ dokusyaRirekiId: 999 }),
        })),
      };
      const getDetailSpy = jest.spyOn(service, 'getDetail');
      const session = sess();
      ...
      await expect(
        service.getEffectiveAt(7, '2026-08-20', session),
      ).rejects.toMatchObject({
        response: expect.objectContaining({
          errors: expect.arrayContaining([
      ...
      // 早期リターン — 直前行探索(loadEffectiveRow)/getDetail まで到達しない。
      expect(getDetailSpy).not.toHaveBeenCalled();
    });
  });
  ...
  describe('create', () => {
    ...
    // ─── 廃店の販売店・失効の単価は新規選択できない (バグ報告2026-08) ────────
    it('should reject creating a subscriber with a closed (haiten_flg=true) hanbaiten', async () => {
      mockBankShitenLookup(true);
      hanbaitenRepo.findOne.mockResolvedValueOnce({
        jaId: fkScopeJaId,
        haitenFlg: true,
      });
      await expect(
    ...
    it('should reject creating a subscriber with an expired (active_flg=false) tanka', async () => {
      mockBankShitenLookup(true);
      tankaRepo.findOne.mockResolvedValueOnce({
        jaId: fkScopeJaId,
        activeFlg: false,
      });
      await expect(
    ...
    it('should ALLOW create with dokusya_kaishi_date=TODAY for 電子版+現金集金 (支払方法問わず・顧客要件 2026-07 改訂)', async () => {
      // 電子版(2) は口座引落に限らずラジオ「今日/翌月1日」で当日可（FE と同一基準）。
      const res = await service.create(
        buildCreateDokusyaBody({
          dokusya_shubetsu: 2,
          shiharai_hoho: 2, // 現金集金（口座引落ではない）
          bank_shiten_id: null,
          dokusya_busu: 1,
          email: 'digi-genkin-today@example.com',
          dokusya_kaishi_date: todayIsoJst(),
        }),
        buildChuokaiSession({ ja_id: 1, account_id: 11 }),
        baseReq,
    ...
    it('should force all 12 配達先 columns blank/true when creating a 電子版 record even if the body carries values (バグ報告2026-08)', async () => {
      // COVERS: DokusyaFormView.vue §7.5 (配達先情報エリア非活性化) — the FE
      // hides the section for 電子版, but the payload is still whatever the
      // client sends. BE must independently gate via buildHaitatsuPayload so
      // a stale/direct-API payload can never persist 配達先 data for 電子版.
      mockBankShitenLookup(true);
      let savedRow: any;
      txManager.save.mockImplementation(async (_entity: any, value: any) => {
        if (value && 'dokusyaBusu' in (value ?? {})) savedRow = value;
      ...
      const persisted = savedRow ?? {};
      expect(persisted.haitatsuSameFlg).toBe(true);
      expect(persisted.haitatsuYubinNo).toBe('');
      expect(persisted.haitatsuTodofukenCode).toBe('');
      expect(persisted.haitatsuShikuchoson).toBe('');
      expect(persisted.haitatsuChomeBanchi).toBe('');
      expect(persisted.haitatsuTatemonoMei).toBe('');
      expect(persisted.haitatsuRenrakusaki1).toBe('');
      expect(persisted.haitatsuRenrakusaki2).toBe('');
      expect(persisted.haitatsuShimeiSei).toBe('');
      expect(persisted.haitatsuShimeiMei).toBe('');
      expect(persisted.haitatsuShimeiKanaSei).toBe('');
      expect(persisted.haitatsuShimeiKanaMei).toBe('');
    });
    ...
    it('should accept create when 電子版(2) even if biko line 1 exceeds 30 chars (行別チェック撤廃・紙版と統一, 顧客要件2026-08-26)', async () => {
      mockBankShitenLookup(true);
    ...
    it('should accept create when 紙版(1) even if biko line 1 exceeds 30 chars (電子版へ連携されないため対象外)', async () => {
      mockBankShitenLookup(true);
  ...
  describe('update', () => {
    ...
    // ─── 廃店の販売店・失効の単価は「変更」だけを拒否する (バグ報告2026-08) ────
    // 既に廃店/失効へ紐づく既存購読者を他項目の編集だけで永久に保存不能に
    // しないよう、hanbaiten_id/tanka_id を変えない限りは通す（assertFkScope
    // の `before` 比較）。
    it('should reject changing hanbaiten_id to a closed (haiten_flg=true) hanbaiten on update', async () => {
      ...
      await expect(
        service.update(
          100,
          buildUpdateDokusyaBody({ hanbaiten_id: 999 }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ),
      ).rejects.toMatchObject({
    ...
    it('should reject changing tanka_id to an expired (active_flg=false) tanka on update', async () => {
      const before = buildDokusya({ dokusyaId: 100, jaId: 1, rirekiNo: 1, tankaId: 1 });
      dokusyaRepo.findOne.mockResolvedValue(before);
      mockBankShitenLookup(true);
      tankaRepo.findOne.mockResolvedValueOnce({
        jaId: fkScopeJaId,
      ...
      await expect(
        service.update(
          100,
          buildUpdateDokusyaBody({ tanka_id: 999 }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ),
      ).rejects.toMatchObject({
    ...
    it('should NOT reject an update that keeps the SAME hanbaiten_id/tanka_id even if they have since become closed/expired', async () => {
      ...
      dokusyaRepo.findOne.mockResolvedValue(before);
      mockBankShitenLookup(true);
      // 既存の hanbaiten_id=5 / tanka_id=1 が事後に廃店/失効した想定。
      hanbaitenRepo.findOne.mockResolvedValueOnce({
        jaId: fkScopeJaId,
        haitenFlg: true,
      });
      ...
      const result = await service.update(
        100,
        buildUpdateDokusyaBody({ hanbaiten_id: 5, tanka_id: 1, dokusya_busu: 2 }),
        buildChuokaiSession({ ja_id: 1, account_id: 11 }),
        baseReq,
      );
    ...
    describe('change_mode (当日変更 / 予約変更)', () => {
      ...
      it('should force all 12 配達先 columns blank/true when updating a 電子版 record even if the body carries values (バグ報告2026-08)', async () => {
        // COVERS: DokusyaFormView.vue §7.5 (配達先情報エリア非活性化) — same
        // gate as create, exercised via the UPDATE path (buildUpdatePartial
        // → buildInsertPayload → buildHaitatsuPayload). dokusya_shubetsu is
        // immutable on edit but the body can still carry stale 配達先 values.
        dokusyaRepo.findOne.mockResolvedValue(
          buildDokusya({
            dokusyaId: 100,
            jaId: 1,
        ...
        expect(values.haitatsuSameFlg).toBe(true);
        expect(values.haitatsuYubinNo).toBe('');
        expect(values.haitatsuTodofukenCode).toBe('');
        expect(values.haitatsuShikuchoson).toBe('');
        expect(values.haitatsuChomeBanchi).toBe('');
        expect(values.haitatsuTatemonoMei).toBe('');
        expect(values.haitatsuRenrakusaki1).toBe('');
        expect(values.haitatsuRenrakusaki2).toBe('');
        expect(values.haitatsuShimeiSei).toBe('');
        expect(values.haitatsuShimeiMei).toBe('');
        expect(values.haitatsuShimeiKanaSei).toBe('');
        expect(values.haitatsuShimeiKanaMei).toBe('');
      });
    ...
    describe('予約変更の同一適用日1回まで制限（顧客要件2026-08）', () => {
      const sess = () => buildChuokaiSession({ ja_id: 1, account_id: 11 });

      function mockExistingChangeAtDate(found: boolean): void {
        dataSource.manager = {
          createQueryBuilder: jest.fn(() => ({
            ...makeQbMock(),
            getOne: jest.fn().mockResolvedValue(found ? { dokusyaRirekiId: 999 } : null),
          })),
        };
      }

      it('紙版・予約変更で同一適用日に既存の変更があれば VALIDATION_ERROR(field=joho_henko_tekiyo_date) で弾く', async () => {
        dokusyaRepo.findOne.mockResolvedValue(
          buildDokusya({ dokusyaId: 100, jaId: 1, dokusyaShubetsu: 1, dokusyaBusu: 1 }),
        );
        mockBankShitenLookup(true);
        mockExistingChangeAtDate(true);
        await expect(
          service.update(
            100,
            buildUpdateDokusyaBody({
      ...
      it('紙版・予約変更で同一適用日に既存の変更が無ければ成功する', async () => {
        dokusyaRepo.findOne.mockResolvedValue(
          buildDokusya({ dokusyaId: 100, jaId: 1, dokusyaShubetsu: 1, dokusyaBusu: 1 }),
        );
        mockBankShitenLookup(true);
        mockExistingChangeAtDate(false);
        const result = await service.update(
          100,
          buildUpdateDokusyaBody({
            change_mode: 'reserved',
      ...
      it('紙版・当日変更は同一適用日に既存の変更があっても制限されない（当日行は取消不可のため）', async () => {
        dokusyaRepo.findOne.mockResolvedValue(
          buildDokusya({ dokusyaId: 100, jaId: 1, dokusyaShubetsu: 1, dokusyaBusu: 1 }),
        );
        mockBankShitenLookup(true);
    ...
    it('(C) should allow 再購読(電子版) with 購読開始日=today — 不具合修正2026-08: 一律 assertTekiyoDateFuture で当日が弾かれていた', async () => {
      const resubSpy = jest
        .spyOn(historyWriter, 'insertResubscribe')
        .mockResolvedValue({
          dokusyaId: 100,
      ...
      try {
        ...
        const result = await service.update(
          ...
          buildUpdateDokusyaBody({
            tetsuzuki_shurui: 1, // 新規（再購読）
            dokusya_busu: 1, // 電子版は購読部数=1固定
            dokusya_kaishi_date: today,
            joho_henko_tekiyo_date: today,
          }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        );
        ...
        expect(resubSpy).toHaveBeenCalledTimes(1);
        expect(resubSpy.mock.calls[0][1]).toEqual(
          expect.objectContaining({ dokusyaId: 100, kaishiDate: today }),
        );
      } finally {
        resubSpy.mockRestore();
      }
    ...
    it('(C) should reject 再購読(電子版) with a past 購読開始日 (当日は許容・過去日のみ不可)', async () => {
      const before = buildDokusya({
        dokusyaId: 100,
        jaId: 1,
        rirekiNo: 2,
    ...
    it('should accept update when 電子版(2) even if biko line 1 exceeds 30 chars (行別チェック撤廃・紙版と統一, 顧客要件2026-08-26)', async () => {
      // 購読種別は編集で不変 → before の版(電子版)で判定。
      const before = buildDokusya({
        dokusyaId: 100,
        jaId: 1,
        rirekiNo: 1,
    ...
    it('should pass johoDate + changed hanbaitenId to applyChange when 販売店 changes on update (販売店適用日=joho に統一)', async () => {
      ...
      // 履歴は1件・joho_henko_tekiyo_date=joho の転記は writer の責務。
      const before = buildDokusya({
        dokusyaId: 100,
        jaId: 1,
        rirekiNo: 1,
    ...
    // undefined）。UI/取込/置換で同一。分割なし・joho_henko_tekiyo_date=joho の転記は
  ...
  // ════════════════════════════════════════════════════════════════════════
  // ACSMS-API-011-007 — PUT /api/v1/dokusya/:dokusya_id/register-tanka
  // ════════════════════════════════════════════════════════════════════════
  describe('registerTanka', () => {
    it('registers tanka_id, keeps denshi_shonin_status=null, and returns 登録しました。', async () => {
      const before = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(before);

      const result = await service.registerTanka(
        100,
        buildChuokaiSession({ ja_id: 1, account_id: 11 }),
        baseReq,
        { tanka_id: 5 },
      ...
      const data = result.data ?? result;
      expect(data.tanka_id).toBe(5);
      expect(data.denshi_shonin_status).toBeNull();
      expect(result.message).toBe('登録しました。');
    });

    it('does not push to denshiban (対象カテゴリは電子版連携で完結・cloud は参照専用)', async () => {
      const before = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(before);

      await service.registerTanka(
        100,
        buildChuokaiSession({ ja_id: 1 }),
        baseReq,
        { tanka_id: 5 },
      ...
      expect(denshiPush.pushOnWrite).not.toHaveBeenCalled();
      expect(denshiPush.push).not.toHaveBeenCalled();
    });

    it('should throw NotFoundException when dokusya does not exist', async () => {
      dokusyaRepo.findOne.mockResolvedValue(null);
      await expect(
        service.registerTanka(999, buildChuokaiSession({ ja_id: 1 }), baseReq, { tanka_id: 5 }),
      ).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException when target belongs to a different ja_id (DataScope)', async () => {
      const otherJa = buildDokusya({
        dokusyaId: 100, jaId: 99, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(otherJa);
      await expect(
        service.registerTanka(100, buildJaHontenSession({ ja_id: 1 }), baseReq, { tanka_id: 5 }),
      ).rejects.toThrow(NotFoundException);
    });

    it('should throw INVALID_STATUS when denshi_shonin_status is not null (承認ワークフロー対象)', async () => {
      const pending = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: 0, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(pending);
      await expect(
        service.registerTanka(100, buildChuokaiSession({ ja_id: 1 }), baseReq, { tanka_id: 5 }),
      ).rejects.toMatchObject({
        response: expect.objectContaining({ error_code: 'INVALID_STATUS' }),
      });
    });

    it('should throw INVALID_STATUS when tanka_id is already registered', async () => {
      const already = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: 3,
      });
      dokusyaRepo.findOne.mockResolvedValue(already);
      await expect(
        service.registerTanka(100, buildChuokaiSession({ ja_id: 1 }), baseReq, { tanka_id: 5 }),
    ...
    it('should reject tanka_id belonging to a different JA (Layer4 FK guard)', async () => {
      const before = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(before);
      tankaRepo.findOne = jest.fn(async () => ({ jaId: 2, activeFlg: true }));
    ...
    it('should insert one history row (tanka_id only), promote it to saishin, and NOT set denshi_shonin_status on t_dokusya update', async () => {
      const before = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(before);
      ...
      const insertedRow = txManager.save.mock.calls
        .map((c: any[]) => c[1] ?? c[0])
        .find(
          (v: any) => v && typeof v === 'object' && v.tankaId === 5 && 'rirekiNo' in v,
        );
      expect(insertedRow).toBeDefined();
      expect(insertedRow.saishinDataFlg).toBe(true);
      expect(insertedRow.johoHenkoTekiyoDate).toBe(todayIsoJst());
      ...
      const dokusyaUpdate = txManager.update.mock.calls.find(
        (c: any[]) =>
          c[1]?.dokusyaId === 100 && c[2] && typeof c[2] === 'object' && 'tankaId' in c[2],
      );
      expect(dokusyaUpdate?.[2].tankaId).toBe(5);
      // denshi_shonin_status キー自体を含めない = master 側は前値(null)のまま。
      expect('denshiShoninStatus' in (dokusyaUpdate?.[2] ?? {})).toBe(false);
    });

    it('should wrap tanka update + audit log in a single transaction', async () => {
      const before = buildDokusya({
        dokusyaId: 100, jaId: 1, denshiShoninStatus: null, tankaId: null,
      });
      dokusyaRepo.findOne.mockResolvedValue(before);
      ...
      expect(dataSource.transaction).toHaveBeenCalledTimes(1);
      expect(auditLog.logUpdate).toHaveBeenCalledTimes(1);
    });
  });
...
describe('DokusyaService — search / delete / export (SCR-014)', () => {
  ...
  describe('search', () => {
    ...
    it('should filter by dokusyaso_bunrui as a CSV token match when provided (顧客CR 2026-08-24)', async () => {
      // COVERS: t_dokusya.dokusyaso_bunrui は CSV（例 "0,1"）— 単純な `=` では
      // なく `,code,` のトークン単位一致であることを検証する。
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);

      await service.search(
        buildSearchDokusyaQuery({ dokusyaso_bunrui: '1' }),
        buildChuokaiSession({ ja_id: 1 }),
      );

      const call = dokusyaQb.andWhere.mock.calls.find(
        ([sql]: unknown[]) =>
          typeof sql === 'string' && sql.includes('dokusyaso_bunrui'),
      );
      expect(call).toBeDefined();
      const [, params] = call as unknown[];
      expect((params as Record<string, unknown>).dokusyasoBunruiPattern).toBe(
        '%,1,%',
      );
    });

    it('should NOT apply the dokusyaso_bunrui filter when omitted', async () => {
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);
      ...
      const applied = dokusyaQb.andWhere.mock.calls.some(
        ([sql]: unknown[]) =>
          typeof sql === 'string' && sql.includes('dokusyaso_bunrui'),
      );
      expect(applied).toBe(false);
    ...
    it('should add range WHERE for 請求開始月 (>= from AND <= to, excluding empty)', async () => {
      // COVERS: 請求開始月 範囲検索 — from ≦ 月 ≦ to、未設定(空文字)は除外
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);
      dokusyaQb.getManyAndCount.mockResolvedValue([[], 0]);
      ...
      const sqls = calls
        ...
        .join(' | ');
      // 未設定(空文字)除外ガード
      expect(/seikyu_kaishi_month\s*<>\s*''/.test(sqls)).toBe(true);
      // from: >= :seikyu_from
      expect(
        calls.some(
          ([sql, p]: any[]) =>
            typeof sql === 'string' &&
            /seikyu_kaishi_month\s*>=/.test(sql) &&
            p?.seikyu_from === '202601',
        ),
      ).toBe(true);
      // to: <= :seikyu_to
      expect(
    ...
    it('should add partial-match WHERE for 引落元口座支店 (bank_branch) across bank_branch_code OR bank_branch_name', async () => {
      // COVERS: §4.3 — bank_branch → (d.bank_branch_code ILIKE ... OR d.bank_branch_name ILIKE ...)
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);
      dokusyaQb.getManyAndCount.mockResolvedValue([[], 0]);

      await service.search(
        buildSearchDokusyaQuery({ bank_branch: '001' }),
        buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
      );
      ...
      const matched = calls.find(
        ([sql, params]: any[]) =>
          ...
          /bank_branch_name/i.test(sql) &&
          /ILIKE/i.test(sql) &&
          params?.bank_branch === '001',
      );
      expect(matched).toBeDefined();
    });

    it('should add partial-match WHERE for 連絡先 (renrakusaki) across renrakusaki_1/2 + haitatsu_renrakusaki_1/2', async () => {
      // COVERS: §4.3 — renrakusaki → (renrakusaki_1 OR haitatsu_renrakusaki_1 OR renrakusaki_2 OR haitatsu_renrakusaki_2) ILIKE
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);
      dokusyaQb.getManyAndCount.mockResolvedValue([[], 0]);

      await service.search(
        buildSearchDokusyaQuery({ renrakusaki: '090' }),
        buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
      );
      ...
      const matched = calls.find(
        ([sql, params]: any[]) =>
          typeof sql === 'string' &&
          /\brenrakusaki_1\b/.test(sql) &&
          /\bhaitatsu_renrakusaki_1\b/.test(sql) &&
          /\brenrakusaki_2\b/.test(sql) &&
          /\bhaitatsu_renrakusaki_2\b/.test(sql) &&
          /ILIKE/i.test(sql) &&
          params?.renrakusaki === '090',
      );
      expect(matched).toBeDefined();
    });

    it('should add exact WHERE for 郵送区分 (yubin_kubun) and 新聞単価 (tanka_id), partial for 備考 (biko)', async () => {
      // COVERS: 郵送区分/新聞単価=完全一致、備考=部分一致（顧客要件 2026-07 追加）
      dokusyaQb.getRawMany.mockResolvedValue([]);
      dokusyaQb.getCount.mockResolvedValue(0);
      dokusyaQb.getManyAndCount.mockResolvedValue([[], 0]);

      await service.search(
        buildSearchDokusyaQuery({ yubin_kubun: '1', tanka_id: 5, biko: 'メモ' }),
        buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
      );
      ...
      // 郵送区分: d.yubin_kubun = :yubin_kubun
      expect(
        calls.some(
          ([sql, p]: any[]) =>
            typeof sql === 'string' &&
            /d\.yubin_kubun\s*=/.test(sql) &&
            p?.yubin_kubun === '1',
        ),
      ).toBe(true);
      // 新聞単価: d.tanka_id = :tanka_id
      expect(
        calls.some(
          ([sql, p]: any[]) =>
            typeof sql === 'string' &&
            /d\.tanka_id\s*=/.test(sql) &&
            p?.tanka_id === 5,
        ),
      ).toBe(true);
      // 備考: d.biko ILIKE
  ...
  describe('exportExcel', () => {
    ...
    it('should write the 15-column Japanese header row mirroring the検索結果テーブル layout', async () => {
      ...
      // 支店 / TEL2 削除、手続種類 / 購読種別 / 配達先氏名 / 支払方法 追加。
      // 連絡先１ → TEL1 (顧客CR 2026-08-24)
      dokusyaQb.getCount.mockResolvedValue(1);
    ...
    it('should NOT include 支店 / TEL2 / かな氏名 columns (removed / search-only fields)', async () => {
      // COVERS: §4.5 — 支店・TEL2 列削除、かな氏名 は検索専用
      dokusyaQb.getCount.mockResolvedValue(1);
      const rows = [buildDokusyaListRow()];
      dokusyaQb.getRawMany.mockResolvedValue(rows);
      dokusyaQb.getManyAndCount.mockResolvedValue([rows, 1]);
      ...
      if (Array.isArray((result as any).headers)) {
        ...
        expect(headers).not.toContain('TEL2');
...
describe('DokusyaService — SCR-015 (replace-hanbaiten search + bulk replace)', () => {
  ...
  describe('searchForReplace', () => {
    ...
    it('should throw DATE_RANGE_INVALID when joho_henko_tekiyo_date is today or past (未来日のみ・顧客要件 2026-07)', async () => {
      // COVERS: 検索段でも置換適用日は未来日のみ。
      primeSearchRows([]);
      await expect(
        service.searchForReplace(
          buildReplaceSearchQuery({ joho_henko_tekiyo_date: '2000-01-01' }),
          buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
        ),
      ).rejects.toMatchObject({ response: { error_code: 'DATE_RANGE_INVALID' } });
    });

    it('should throw VALIDATION_ERROR (field=dokusya_shubetsu) when 電子版(2) is requested — 本画面の対象外 (ACSMS-MSG-015-009・顧客要件 2026-07 改訂)', async () => {
      // COVERS: 電子版=電子配信で販売店を持たない → 一括置換の対象外。FE の検索ボタン
      // 無効化に対する防御的サーバ側ガード（種別が2なら日付に関係なく拒否）。
      primeSearchRows([]);
      await expect(
        service.searchForReplace(
          buildReplaceSearchQuery({
    ...
    it('should throw VALIDATION_ERROR (field=new_hanbaiten_id) when the target is the denshi dummy store (紙版限定画面, 不具合修正2026-08)', async () => {
      // COVERS: ダミー販売店(9999999999)は紙版限定の本画面では置換先になり得ない。
      primeSearchRows([]);
      dataSource.query.mockImplementation(async (sql: any) => {
        if (/from\s+m_hanbaiten/i.test(String(sql ?? ''))) {
          return [{ hanbaiten_id: 201, hanbaiten_code: HANBAITEN_DUMMY_CODE }];
        }
        return [];
      });
      ...
      await expect(
        service.searchForReplace(
          buildReplaceSearchQuery({ new_hanbaiten_id: 201 }),
          buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
        ),
      ).rejects.toMatchObject({
        response: {
          error_code: 'VALIDATION_ERROR',
          errors: [
            expect.objectContaining({ field: 'new_hanbaiten_id' }),
          ],
        },
      });
    });

    it('should throw VALIDATION_ERROR (field=hanbaiten_id) when the source filter is the denshi dummy store (紙版限定画面, 不具合修正2026-08)', async () => {
      // COVERS: 配達販売店(絞り込み)側もダミーは対象外。
      primeSearchRows([]);
      dataSource.query.mockImplementation(async (sql: any) => {
        if (/from\s+m_hanbaiten/i.test(String(sql ?? ''))) {
          return [{ hanbaiten_id: 200, hanbaiten_code: HANBAITEN_DUMMY_CODE }];
        }
        return [];
      });
      ...
      await expect(
        service.searchForReplace(
          buildReplaceSearchQuery({ hanbaiten_id: 200 }),
          buildSession({ ja_id: null, role_code: 'NICHINO_ADMIN' }),
        ),
      ).rejects.toMatchObject({
        response: {
          error_code: 'VALIDATION_ERROR',
          errors: [expect.objectContaining({ field: 'hanbaiten_id' })],
        },
      });
    });
  ...
  describe('replaceHanbaiten', () => {
    ...
    it('should return the summary + message and commit when all rows are eligible', async () => {
      ...
      const result = await service.replaceHanbaiten(
        buildReplaceBody({
          ...
          joho_henko_tekiyo_date: tekiyoDate,
        }),
        buildChuokaiSession({ ja_id: 1, account_id: 11 }),
        baseReq,
      );
    ...
    it('should throw VALIDATION_ERROR (field=new_hanbaiten_id) when the target is the denshi dummy store (紙版限定画面, 不具合修正2026-08)', async () => {
      // COVERS: ダミー販売店(9999999999)は紙版限定の本画面では置換先になり得ない。
      // ドロップダウンでは除外済みだが API を直接叩けば回避できるためサーバ側でも検証。
      const rows = [buildReplaceCandidateRow({ dokusya_id: 5001, hanbaiten_id: 200 })];
      primeReplace(rows);
      dataSource.query.mockImplementation(async (sql: any) => {
        if (/from\s+m_hanbaiten/i.test(String(sql ?? ''))) {
          return [{ hanbaiten_id: 201, ja_id: 1, hanbaiten_code: HANBAITEN_DUMMY_CODE }];
        }
        return [];
      });
      ...
      await expect(
        ...
        response: {
          error_code: 'VALIDATION_ERROR',
          errors: [expect.objectContaining({ field: 'new_hanbaiten_id' })],
        },
      });
    });

    // 紙版の予約変更（未来日）は同一適用日への変更を1回までに制限する（顧客要件2026-08）。
    // 本画面は常に紙版・未来日必須なので、候補全件が対象になる。
    it('should throw INELIGIBLE_DOKUSYA listing the conflicting ids when a candidate already has an active row at this joho date', async () => {
      const rows = [
        buildReplaceCandidateRow({ dokusya_id: 5001, hanbaiten_id: 200 }),
        buildReplaceCandidateRow({ dokusya_id: 5002, hanbaiten_id: 200 }),
      ];
      primeReplace(rows);
      // t_dokusya_rireki 衝突クエリだけ上書き — 5001 が同一適用日に既に変更あり。
      const baseRoute = dataSource.query.getMockImplementation();
      dataSource.query.mockImplementation(async (sql: any, params: any) => {
        const text = String(sql ?? '');
        if (/from\s+t_dokusya_rireki/i.test(text)) {
          return [{ dokusya_id: 5001 }];
        }
    ...
    it('should throw VALIDATION_ERROR (field=dokusya_shubetsu) + NOT fetch candidates when 電子版(2) is requested — 本画面の対象外 (ACSMS-MSG-015-009・顧客要件 2026-07 改訂)', async () => {
      // COVERS: 電子版は一括置換の対象外。候補取得(find)より前に拒否する
      // 防御的サーバ側ガード。
      dokusyaRepo.find.mockClear();
      await expect(
        service.replaceHanbaiten(
          buildReplaceBody({
            dokusya_ids: [5001],
      ...
      expect(dokusyaRepo.find).not.toHaveBeenCalled();
    });
    ...
    it('should record the screen name "統廃合販売店読者移行画面 (ACSMS-SCR-015)" when writing the audit log', async () => {
      ...
      const screenOk = allCtx.some((c: any) =>
        [c?.gamenName, c?.gamen_name, c?.screen, c?.screenName].some(
          (v: any) =>
            typeof v === 'string' &&
            v.includes('統廃合販売店読者移行画面 (ACSMS-SCR-015)'),
        ),
      );
    ...
    it('should reject a 併読 (dokusya_shubetsu=3) candidate via 種別不一致 (DATE_RANGE_INVALID) — 併読 は種別で送れないため一致チェックで弾く', async () => {
      // COVERS: §4.1 候補種別の整合チェック — 併読(3) は dto.dokusya_shubetsu(1/2) と
      // 一致しないため mismatch で弾かれる（顧客要件 2026-07）。
      const rows = [
        buildReplaceCandidateRow({
          dokusya_id: 5001,
          hanbaiten_id: 200,
      ...
      await expect(
        service.replaceHanbaiten(
          buildReplaceBody({
            ...
            new_hanbaiten_id: 201,
            joho_henko_tekiyo_date: tekiyoDate,
            dokusya_shubetsu: 1, // 紙版で検索 → 併読候補は不一致
          }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ),
    ...
    it('should throw VALIDATION_ERROR (電子版は本画面では対象外) when a 電子版クレカ candidate is requested (dokusya_shubetsu=2) — 電子版全体が本画面の対象外 (顧客要件 2026-07 改訂)', async () => {
      // COVERS: 電子版クレカ(2 && 6) は 電子版 の部分集合。1.4 では per-row の
      // INELIGIBLE_DOKUSYA に到達したが、1.5 で 電子版(2) 全体が本画面の対象外と
      // なったため、候補取得より前に VALIDATION_ERROR で拒否される（per-row の
      // ineligible 判定 = isDigitalCreditCard には到達しない）。
      const rows = [
        buildReplaceCandidateRow({
          dokusya_id: 5002,
          hanbaiten_id: 200,
    ...
    it('should reject the request when joho_henko_tekiyo_date is past/today (未来日のみ・顧客要件 2026-07)', async () => {
      ...
      await expect(
        service.replaceHanbaiten(
          buildReplaceBody({
            dokusya_ids: [5001],
            new_hanbaiten_id: 201,
            joho_henko_tekiyo_date: pastDate(7),
          }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ),
...
describe('DokusyaService — SCR-016 (Excel import: template + bulk import)', () => {
  ...
  function buildSqlRouter(over: {
    ...
    // 電子版インポートは行ごと hanbaiten_code をダミー販売店へ強制するため
    // （顧客要件2026-08）、既定のハンバイテン一覧にもダミー行を含めておく —
    // 紙版テストは H001 を、電子版テストはダミーコードを引く。
    const hanbaiten = over.hanbaiten ?? [
      { hanbaiten_id: 5, hanbaiten_code: 'H001' },
      { hanbaiten_id: 9999, hanbaiten_code: HANBAITEN_DUMMY_CODE },
    ];
    // paper_flg/denshi_flg both true by default — [layer4-scope-guard]
    // 不具合修正2026-08 で m_kanri_shiten 自体の取扱いフラグを見るようになった
    // ため、既定は両方許可にして既存の NEW/UPDATE happy-path テストを素通りさせる。
    // 制限を検証する専用テストは over.kanriShiten で上書きする。
    const kanriShiten = over.kanriShiten ?? [
      { kanri_shiten_id: 101, kanri_shiten_code: 'KS001', paper_flg: true, denshi_flg: true },
    ];
    // shiten クエリは m_shiten を m_kanri_shiten へ LEFT JOIN するため
    // kanri_shiten_code も返す（既定は上の kanriShiten[0] の所属先）。
    const shiten = over.shiten ?? [
      { shiten_id: 1001, shiten_code: 'SH001', kanri_shiten_code: 'KS001' },
    ];
  ...
  describe('importExcel — NEW mode happy path', () => {
    ...
    // 不具合修正2026-08 — 電子版は1行=1txにして、書込み段で1行失敗しても
    // 他行の成功済み分をロールバックしない（denshiban 側の孤児レコード問題の
    // 根本対策）。紙版は importExcelAtomic のまま全行1tx（別途カバー済み）。
    describe('電子版の部分成功（1行=1tx・不具合修正2026-08）', () => {
      it('should NOT roll back already-succeeded rows when a later row fails to write (電子版のみ)', async () => {
        primeImport();
        // row1・row3 は既定の成功実装、row2 だけ書込み失敗させる。
        applyChangeSpy
          .mockImplementationOnce(async (_m: any, input: any) => ({
            dokusyaId: 1,
            insertedRirekiIds: [1],
            before: null,
            after: {
          ...
          .mockRejectedValueOnce(new Error('denshiban push failed for row2'));
        // row3 以降は beforeEach の既定 mockImplementation にフォールバック。
        ...
        const result = await service.importExcel(
          buildImportBody({
            ...
            dokusya_shubetsu: 2,
            rows: [
              buildImportRow({ shiharai_hoho: 2, kumiaiin_code: 'D1', email: 'd1@example.com' }),
              buildImportRow({ shiharai_hoho: 2, kumiaiin_code: 'D2', email: 'd2@example.com' }),
              buildImportRow({ shiharai_hoho: 2, kumiaiin_code: 'D3', email: 'd3@example.com' }),
            ],
          }),
          buildJaHontenSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ...
        // row1・row3 は成功しており applyChange が計3回呼ばれている
        // （失敗した row2 も試行はされるので3回— 成功したのはそのうち2件）。
        expect(applyChangeInputs()).toHaveLength(3);

        expect(result.data.total_rows).toBe(3);
        expect(result.data.created_count).toBe(2);
        expect(result.data.failed_count).toBe(1);
        expect(result.data.rireki_count).toBe(2);
        expect(result.row_errors).toEqual([
          { row: 3, message: 'denshiban push failed for row2' },
        ]);
        expect(result.message).toBe('2件成功、1件失敗しました。');
      });

      it('should return created_count=0/failed_count=total when every row fails to write (電子版のみ)', async () => {
        primeImport();
        applyChangeSpy.mockRejectedValue(new Error('always fails'));
        ...
        expect(result.row_errors).toHaveLength(2);
      });

      it('should write ONE summary audit log with result_status=WARNING(3) when some rows fail (電子版のみ)', async () => {
        primeImport();
        applyChangeSpy
          .mockImplementationOnce(async (_m: any, input: any) => ({
            dokusyaId: 1,
          ...
          .mockRejectedValueOnce(new Error('row2 failed'));
        ...
        // §4.7 相当 — importExcel 自体は throw しないため logError ではなく
        // logOperation 1件（resultStatus=3=WARNING）で部分成功を記録する。
        expect(auditLog.logError).not.toHaveBeenCalled();
        const warningLog = auditLog.logOperation.mock.calls.find(
          (c: unknown[]) => (c[0] as Record<string, unknown>)?.resultStatus === 3,
        );
        expect(warningLog).toBeDefined();
      });

      it('should still perform each row in its own dataSource.transaction (per-row isolation, 電子版のみ)', async () => {
        primeImport();
        ...
        // 2行 → dataSource.transaction が最低2回（行ごと）+ 監査ログ用の
        // 追加呼び出しは無い（監査はループの外・tx 無しで1回だけ書く設計）。
        expect(dataSource.transaction.mock.calls.length).toBeGreaterThanOrEqual(2);
      });
    });
  });
  ...
  describe('importExcel — row-level validation (IMPORT_VALIDATION_ERROR)', () => {
    ...
    it('should throw IMPORT_VALIDATION_ERROR with field=kumiaiin_code when a 一括中止 row references an unknown kumiaiin_code', async () => {
      // COVERS: §4.3.4 — 一括中止 via kumiaiin_code 未ヒット。一括中止は
      // payload直下の dokusya_chushi_date で判定する（不具合修正2026-08：
      // dokusya_id 無し行は通常更新では一切 kumiaiin_code フォールバックを
      // 使わず「IDは必須です」になるため、一括中止と明示する必要がある）。
      primeImport({ existing: [] });
    ...
    it('should throw IMPORT_VALIDATION_ERROR (field=kumiaiin_code) when a 一括中止 keys by kumiaiin_code that matches MORE THAN ONE record', async () => {
      // 組合員コードは重複可。dokusya_id 無しで kumiaiin キーが複数ヒットすると
      // 一括誤解約になるため、ID 指定を促すエラーにする。
      primeImport({
        existing: [
...
describe('DokusyaService — rireki UI↔Excel取込 同一性 (parity)', () => {
  ...
  function importRoute(over: { existing?: any[]; onUpdate?: () => any[] } = {}) {
    ...
    return async (sql: unknown): Promise<any[]> => {
      ...
      if (/from\s+m_kanri_shiten/i.test(t))
        return [{ kanri_shiten_id: 101, kanri_shiten_code: 'KS001', paper_flg: true, denshi_flg: true }];
      if (/from\s+m_shiten/i.test(t))
        return [{ shiten_id: 1001, shiten_code: 'SH001', kanri_shiten_code: 'KS001' }];
  ...
  beforeEach(() => {
    ...
    dataSource = {
      transaction: jest.fn(async (cb: any) => cb(txManager)),
      query: jest.fn(async () => []),
      // [reserved-same-date] assertReservedSameDateAvailable calls
      // loadActiveRowAtDate(this.dataSource.manager, ...) pre-transaction.
      // Default: no colliding row (this suite's parity tests don't exercise it).
      manager: {
        createQueryBuilder: jest.fn(() => ({
          ...qb(),
          getOne: jest.fn().mockResolvedValue(null),
    ...
    // Shared by kanriShitenRepo/hanbaitenRepo/tankaRepo below — include both
    // flags (open store / active price defaults) so assertFkScope's
    // closed-hanbaiten/expired-tanka guards don't misfire.
    const fkRepo = {
      findOne: jest.fn(async () => ({ jaId: 1, haitenFlg: false, activeFlg: true })),
    };
    const accountRepo = {
      findOne: jest.fn(async () => ({ accountId: 1, paperFlg: true, denshiFlg: true })),
    };
