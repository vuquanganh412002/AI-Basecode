...
describe('DokusyaService — SCR-011 (create + update + approve/reject + history + detail)', () => {
  ...
  describe('create', () => {
    ...
    it('should throw VALIDATION_ERROR (field=shiharai_hoho) when 電子版 with a non-口座引落 payment method', async () => {
      // COVERS: screen-design §7.3 — 電子版 (dokusya_shubetsu=2) only
      // allows 口座引落 (shiharai_hoho=1) on create.
      await expect(
        service.create(
          buildCreateDokusyaBody({ dokusya_shubetsu: 2, shiharai_hoho: 2 }),
          buildChuokaiSession({ ja_id: 1, account_id: 11 }),
          baseReq,
        ),
      ).rejects.toMatchObject({
    ...
    it('should set denshi_shonin_status=2 (否認) when creating a 紙版 record', async () => {
      // 紙版 is not part of the web-application 承認/否認 flow → status 2.
      const result = await service.create(
        buildCreateDokusyaBody({
          dokusya_shubetsu: 1,
          shiharai_hoho: 2,
      ...
      expect(result.denshi_shonin_status).toBe(2);
    });
