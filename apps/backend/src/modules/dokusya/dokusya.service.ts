...
export class DokusyaService {
   ...
   * 即時反映のため変更は常に本日適用、予約変更(未来日)は不可。FE は電子版でも紙版と同じ
   * モードバーを出すが「予約変更」ボタンを disabled にし、selectMode 側でも弾いている
   * （UI 統一・2026-07 改訂）。改竄/退行に備え BE でも弾く
   * （購読種別は before の保存値で判定・spoof 不可）。
