...
/** YYYY-MM-DD literal (ISO 8601 date-only). */
...
export class CreateDokusyaDto {
  ...
  @ApiPropertyOptional({ description: '支店ID (FK: m_shiten)' })
  @IsOptional()
  @Type(() => Number)
  ...
  @ApiPropertyOptional({ description: '配達先 郵便番号', maxLength: 7 })
  @Transform(blankToUndef)
  @IsOptional()
  @IsString({ message: '配達先 郵便番号は文字列で指定してください。' })
  @MaxLength(7, { message: '配達先 郵便番号は最大7文字で指定してください。' })
