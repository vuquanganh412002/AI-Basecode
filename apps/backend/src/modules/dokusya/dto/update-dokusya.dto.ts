 ...
 * `@IsEmpty()` パターンで拒否し、ボディに紛れ込ませたクライアントには
 * `dokusya_id` フィールドエラーの 400 を返す（URL パスパラメータが唯一の真実）。
 *
 * `PartialType(CreateDokusyaDto)` は使わない — 更新時も画面はフォーム全体を送信し、
 * 全必須項目を必須のまま維持することで FE の検証面が作成・更新で一貫する。
 */
export class UpdateDokusyaDto extends CreateDokusyaDto {
  /**
   * `dokusya_id` は URL パス上にある — ボディで持ってはならない。
   * `@IsEmpty()` の理由は create-dokusya.dto.ts 参照。
   */
