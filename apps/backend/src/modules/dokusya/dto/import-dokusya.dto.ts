...
/** Coerce blank strings to undefined so `@IsOptional()` skips them. */
 ...
 * `@Type(() => Number)` so a JSON-string survives into a number;
 * string fields carry the api.md §リクエストパラメータ max-lengths.
 */
