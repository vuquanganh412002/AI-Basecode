...
describe('UpdateTankaDto', () => {
  ...
  // ─── campaign_flg (Boolean, optional, inherited from CreateTankaDto) ─────
  it('should accept campaign_flg omitted', async () => {
    const dto = plainToInstance(
      UpdateTankaDto,
      buildUpdateTankaPayload({ campaign_flg: undefined }),
    );
    const errors = await validate(dto);
  ...
  it('should accept campaign_flg=true', async () => {
    const dto = plainToInstance(
      UpdateTankaDto,
      buildUpdateTankaPayload({ campaign_flg: true }),
    );
    const errors = await validate(dto);
    expect(errors.filter((e) => e.property === 'campaign_flg')).toHaveLength(0);
  });

  it('should fail when campaign_flg is non-boolean string', async () => {
    const dto = plainToInstance(
      UpdateTankaDto,
      buildUpdateTankaPayload({ campaign_flg: 'maybe' }),
    );
    const errors = await validate(dto);
