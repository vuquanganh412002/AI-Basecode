...
describe('SearchTankaDto', () => {
  ...
  // ─── campaign_flg (Boolean, optional) ────────────────────────────────────
  it('should coerce campaign_flg from "1" string to boolean true', async () => {
    // COVERS: FE sends `'1'` / `'0'` (HTML select) — DTO accepts and normalizes
    const dto = plainToInstance(SearchTankaDto, { campaign_flg: '1' });
    const errors = await validate(dto);
    expect(errors.filter((e) => e.property === 'campaign_flg')).toHaveLength(0);
    expect(dto.campaign_flg).toBe(true);
  });

  it('should coerce campaign_flg from "0" string to boolean false', async () => {
    const dto = plainToInstance(SearchTankaDto, { campaign_flg: '0' });
    const errors = await validate(dto);
    expect(errors.filter((e) => e.property === 'campaign_flg')).toHaveLength(0);
    expect(dto.campaign_flg).toBe(false);
  });

  it('should accept campaign_flg omitted (filter ignored = both states returned)', async () => {
    const dto = plainToInstance(SearchTankaDto, {});
    const errors = await validate(dto);
    expect(errors.filter((e) => e.property === 'campaign_flg')).toHaveLength(0);
    expect(dto.campaign_flg).toBeUndefined();
  });
