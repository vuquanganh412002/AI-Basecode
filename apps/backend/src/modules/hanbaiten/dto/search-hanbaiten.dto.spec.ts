...
describe('SearchHanbaitenDto', () => {
  ...
  describe('happy path', () => {
    ...
    it('should default sort_by=hanbaiten_code when sort_by is omitted', async () => {
      const { dto } = await run({});
      expect((dto as any).sort_by).toBe('hanbaiten_code');
    });
  ...
  // ─── sort_by ────────────────────────────────────────────────────────
  describe('sort_by', () => {
    it.each([['hanbaiten_code'], ['hanbaiten_name']])(
      'should pass when sort_by = %s (画面設計書 v1.2 §8.1 allow-list)',
      async (val) => {
