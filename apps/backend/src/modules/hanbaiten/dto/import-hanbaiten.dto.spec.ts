...
describe('ImportHanbaitenDto', () => {
  ...
  describe('rows[i] — per-field length/type checks', () => {
    ...
    it('should reject when tesuryo_amount is negative', async () => {
      const errs = await check({
        ...VALID,
        rows: [buildImportRow({ tesuryo_amount: -1 })],
      });
      const flat = flattenProperties(errs);
      expect(flat.some((p) => /rows.*tesuryo_amount/.test(p))).toBe(true);
    });
    ...
    it('should reject when haiten_flg is a string instead of boolean', async () => {
      const errs = await check({
        ...VALID,
