...
describe('ImportHanbaitenDto', () => {
  ...
  // ─── Excel numeric cells in VARCHAR columns (Fix #1) ──────────────────
  // Excel stores numeric-looking values (郵便番号, 金融機関コード, 口座番号,
  // 口座支店コード) as JS numbers, so sheet_to_json hands them to the DTO as
  // `number`. The blankToUndef transform coerces number → string so a bare
  // @IsString does not reject the row (which would collapse to one generic
  // "取込データ / 入力値が不正です" line). Regression for テンプレート(18).xlsx.
  describe('rows[i] — Excel numeric cells coerced to string', () => {
    it('should accept numeric yubin_no / bank_code / koza_no / bank_branch_code (Excel number cells)', async () => {
      const errs = await check({
        ...VALID,
        rows: [
          {
            hanbaiten_code: 'NUM01',
            yubin_no: 1000012,
            bank_code: 4445,
            bank_branch_code: 334,
            koza_no: 1233,
          },
        ],
      });
      const flat = flattenProperties(errs);
      expect(flat.some((p) => p.startsWith('rows.0.'))).toBe(false);
    });

    it('should accept a numeric hanbaiten_code (Excel number cell) by coercing to string', async () => {
      const errs = await check({
        ...VALID,
        rows: [{ hanbaiten_code: 12345 }],
      });
      const flat = flattenProperties(errs);
      expect(flat.some((p) => /rows.*hanbaiten_code/.test(p))).toBe(false);
    });

    it('should still reject a numeric yubin_no whose digit count is not 7 after coercion', async () => {
      const errs = await check({
        ...VALID,
        rows: [{ hanbaiten_code: 'NUM02', yubin_no: 12345 }],
      });
      const flat = flattenProperties(errs);
      expect(flat.some((p) => /rows.*yubin_no/.test(p))).toBe(true);
    });
