...
describe('CreateHanbaitenDto', () => {
  ...
  describe('itaku_kubun / tesuryo_kubun / yokin_shubetsu (numeric m_code values)', () => {
    ...
    it('should reject when tesuryo_kubun is a non-numeric string', async () => {
      const errs = await check({ ...VALID, tesuryo_kubun: 'xyz' });
      expect(errs.some((e) => e.property === 'tesuryo_kubun')).toBe(true);
    });
  ...
  describe('tesuryo_amount (optional, >= 0)', () => {
    it('should accept when tesuryo_amount is 0', async () => {
      const errs = await check({ ...VALID, tesuryo_amount: 0 });
      expect(errs.some((e) => e.property === 'tesuryo_amount')).toBe(false);
    });

    it('should reject when tesuryo_amount is negative', async () => {
      const errs = await check({ ...VALID, tesuryo_amount: -1 });
      expect(errs.some((e) => e.property === 'tesuryo_amount')).toBe(true);
    });

    it('should accept when tesuryo_amount is omitted', async () => {
      const { tesuryo_amount: _drop, ...without } = VALID;
      expect((await check(without)).some((e) => e.property === 'tesuryo_amount')).toBe(false);
    });
  });
