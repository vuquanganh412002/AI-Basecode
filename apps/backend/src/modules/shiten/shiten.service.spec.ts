...
describe('ShitenService — SCR-006 (list / delete)', () => {
  ...
  describe('findAll (API-006-001)', () => {
    ...
    it('should apply DataScope (ja_id = session.ja_id) when caller is JA_KANRI_SHITEN', async () => {
      // SCR-006 scope is by JA only (per screen-design §1.1 — JA管理支店:
      // 自JAのレコードのみ; NOT scoped down to kanri_shiten level).
