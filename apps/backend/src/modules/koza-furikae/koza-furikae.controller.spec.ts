...
describe('KozaFurikaeController (HTTP)', () => {
  ...
  describe('POST /api/v1/koza-furikae/export', () => {
    it('should return 200 with a text/csv attachment when データ exists', async () => {
      service.exportCsv.mockResolvedValue({
        buffer: Buffer.from('1,21,0,...'),
        filename: 'koza_furikae_20260522_103000.csv',
        recordCount: 2,
      });
      ...
      expect(res.headers['content-disposition']).toContain('koza_furikae_20260522_103000.csv');
    });
