...
export class KanriShitenController {
  ...
  async listDropdown(
    ...
    const data = await this.service.listDropdown(query.ja_id);
    return { data };
  }
  ...
  @ApiResponse({ status: 200, type: KanriShitenDetailDto })
  async findById(
  ...
  @ApiResponse({ status: 201, type: KanriShitenDetailDto })
  async create(
