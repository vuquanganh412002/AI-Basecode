...
const config: Config = {
  ...
  // Resolve `@/...` and `@test/...` paths matching tsconfig.json `paths`.
  moduleNameMapper: {
    '^@/(.*)$': '<rootDir>/src/$1',
    '^@test/(.*)$': '<rootDir>/test/$1',
  },
  ...
  coverageThreshold: {
    ...
    'src/modules/log/**/*.ts': {
      // SCR-030 — list + CSV export. Branches dominated by DataScope
      // role switch (5 roles) + date-range guards + EXPORT_LIMIT_EXCEEDED
      // path + audit-log error-log catch.
      statements: 97,
      functions: 98,
      lines: 97,
      branches: 73,
    ...
    'src/modules/health/**/*.ts': {
      // Liveness + readiness — controller-only, no business branches.
      statements: 100,
      functions: 100,
      lines: 100,
      branches: 100,
    ...
    'src/modules/mail/**/*.ts': {
      // Provider selection (SES vs SMTP) + template rendering + send().
      statements: 100,
      functions: 100,
      lines: 100,
      branches: 100,
    ...
    'src/modules/redis/**/*.ts': {
      // ioredis wrapper — covered with ioredis-mock end-to-end.
      statements: 100,
      functions: 100,
      lines: 100,
      branches: 100,
