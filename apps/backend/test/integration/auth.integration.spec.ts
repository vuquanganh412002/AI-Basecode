...
describe('Auth + Oshirase modules — integration (pg-mem + ioredis-mock)', () => {
  ...
  // ───────────────────────────────────────────────────────────────────
  // GET /api/v1/oshirase/public (API-001-006) — 認証不要
  // ───────────────────────────────────────────────────────────────────
  describe('GET /api/v1/oshirase/public', () => {
    it('should return 200 with public notices ordered by publish_start_date DESC when called without auth', async () => {
      const res = await http()
      ...
      // Most recent publish_start_date first.
      const dates = res.body.data.map((n: any) => n.publish_start_date);
      const sorted = [...dates].sort().reverse();
      expect(dates).toEqual(sorted);
    });

    it('should return 200 with empty array when publish_location=2 has no notices', async () => {
      const res = await http()
        .get('/api/v1/oshirase/public?publish_location=2')
        .expect(200);

      expect(res.body.data).toEqual([]);
    });

    it('should return 400 VALIDATION_ERROR when limit > 10', async () => {
      await http()
        .get('/api/v1/oshirase/public?limit=99')
        .expect(400)
        .expect((res) => {
          expect(res.body.error_code).toBe('VALIDATION_ERROR');
        });
