...
describe('JA module — integration (pg-mem + ioredis-mock)', () => {
  ...
  beforeEach(async () => {
    ctx = await createIntegrationTestApp({
      ...
      seedSql: [
        ...
        `INSERT INTO m_ja
         ...
         VALUES
          ('1301001001', 'JA東京中央', 'ジェイエイトウキョウチュウオウ', '13', true,
           '1234', '農林中央金庫', '1000001', '東京都千代田区丸の内1-1-1',
        ...
        // single FK-bearing column the service's COUNT(*) needs. Replace with
        // full entity definitions once those modules ship.
        `CREATE TABLE IF NOT EXISTS m_kanri_shiten (
           kanri_shiten_id SERIAL PRIMARY KEY,
           ja_id BIGINT NOT NULL,
           deleted_at TIMESTAMPTZ NULL
         )`,
        `CREATE TABLE IF NOT EXISTS m_shiten (
           shiten_id SERIAL PRIMARY KEY,
           ja_id BIGINT NOT NULL,
           deleted_at TIMESTAMPTZ NULL
         )`,
        `CREATE TABLE IF NOT EXISTS m_hanbaiten (
           hanbaiten_id SERIAL PRIMARY KEY,
           ja_id BIGINT NOT NULL,
           deleted_at TIMESTAMPTZ NULL
         )`,
        `CREATE TABLE IF NOT EXISTS m_tanka (
           tanka_id SERIAL PRIMARY KEY,
           ja_id BIGINT NOT NULL,
           deleted_at TIMESTAMPTZ NULL
         )`,
  ...
  it('should silently ignore non-allow-list fields when CHUOKAI updates (※4 field-level restriction)', async () => {
    ...
    const body = {
      ...
      bank_code: '9999', // not in CHUOKAI allow-list
      yubin_no: '1000001',
      address: '東京都千代田区丸の内2-2-2',
      tel: '0312345678',
      fax: '0312345679',
    ...
    const [row] = await ctx.dataSource.query(
      `SELECT ja_name, bank_code, address FROM m_ja WHERE ja_id = 1`,
    );
    expect(row.ja_name).not.toBe('ATTEMPT_TO_CHANGE');
    expect(row.bank_code).not.toBe('9999');
  ...
  describe('API-004-002 — DELETE /api/v1/ja/:ja_id', () => {
    it('should soft-delete and write t_log DELETE row in same tx when NICHINO_ADMIN deletes a JA with no related rows', async () => {
      ...
      expect(res.body.message).toBe('正常に削除しました。');
    ...
    it('should return 409 CONFLICT when m_kanri_shiten has related rows', async () => {
      ...
      await ctx.dataSource.query(`INSERT INTO m_kanri_shiten (ja_id) VALUES (1)`);
      const sid = await asAdmin();
