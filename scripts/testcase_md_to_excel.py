...
def _parse_case_body(body: str) -> TestCase:
    ...
    while i < len(body_lines):
        ln = body_lines[i]
        # Accept BOTH `- **field**:` (legacy SKILL output) and `- field:`
        # (post-cleanup, no bold). The `**` markers are optional. Anchor
        # to start-of-line (no leading indent) so indented sub-bullets
        # like `  - ・role: NICHINO_ADMIN` aren't mis-read as new fields.
        m = re.match(r"^-\s+\*{0,2}([^\*:][^:\*\n]*?)\*{0,2}\s*:\s*(.*)$", ln)
        if not m:
            i += 1
            continue
