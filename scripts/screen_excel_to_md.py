...
def _find_message_header(ws, search_max=40):
    """
    Locate the メッセージ情報 table header by scanning for the row that
    carries the 'メッセージコード' label, and read the actual column of
    each header cell from that row.

    Hardcoded positions are not reliable here: across the customer
    workbooks the header sits at row 8, 9, or 10, and メッセージ内容 lands
    at column G (7) in most screens but H (8) in others (e.g. SCR-007,
    where メッセージコード is merged C:G so a fixed read at column 7 would
    return the code value instead of the content). Detecting the columns
    from the label row makes the converter robust to both variants.

    Returns (header_row, num_col, code_col, content_col) or None.
    """
    for r in range(1, search_max + 1):
        num_col = code_col = content_col = None
        for c in range(1, 25):
            v = ws.cell(r, c).value
            if v is None:
                continue
            ...
            if 'メッセージコード' in s:
                ...
                num_col = c
        if code_col is not None and content_col is not None:
            if num_col is None:
                num_col = max(1, code_col - 1)
            return r, num_col, code_col, content_col
    return None
...
def convert_messages(ws, layout, origins, merged_values):
    ...
    detected = _find_message_header(ws)
    if detected is not None:
        header_row, num_col, code_col, content_col = detected
        cols = [('#', num_col), ('メッセージコード', code_col),
                ('メッセージ内容', content_col)]
    else:
        # Fallback to the layout's fixed positions when no header is found.
